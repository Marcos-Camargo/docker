...
function getThumbnailUrl($path, $fileName,$home) {
    ...
    if (generateThumbnail($sourceFile, $targetFile) === true) {
        return $home.'assets/images/catalog_product_image/thumbs/' . $fileName;
    } else {
        return $home.'assets/images/catalog_product_image/' . $fileName; // return the original file
    }
}
...
function upload() {
    ...
    $targetDir = $serverpath . 'assets/images/catalog_product_image';
    ...
    if (isset($_FILES[$fileBlob]) && isset($_POST['uploadToken'])) {
        ...
        if($upload['success']){
            ...
            $zoomUrl = $home . 'assets/images/catalog_product_image/' . $token ."/" . $upload['path'];
            $fileSize = filesize('./../../../images/catalog_product_image/' . $token ."/" . $upload['path']);
            return [
            ...
            return [
                'error' => 'Error uploading chunk ' . $_POST['chunkIndex'] . json_encode($upload)
            ];
        }
    }
    return [
