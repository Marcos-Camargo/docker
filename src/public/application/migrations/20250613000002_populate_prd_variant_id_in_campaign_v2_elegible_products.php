...
return new class extends CI_Migration
{
    public function up() {
        ...
        // Get all records from campaign_v2_elegible_products
        $this->db->select('campaign_v2_elegible_products.*');
        $this->db->where('products.has_variants !=', '');
        $this->db->join('products', 'products.id = campaign_v2_elegible_products.product_id');
        $query = $this->db->get('campaign_v2_elegible_products');
        ...
        foreach ($products as $product) {
            ...
            if (!empty($variants)) {
                foreach ($variants as $variant) {
                    ...
                    $this->db->insert('campaign_v2_elegible_products', $new_record);
                }

                // Delete the original record after all variants have been inserted
                $this->db->where('id', $product['id']);
                $this->db->delete('campaign_v2_elegible_products');
            }
        }
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            log_message('error', 'Migration 20250613000002: Failed to populate prd_variant_id in campaign_v2_elegible_products');
        }
    }
