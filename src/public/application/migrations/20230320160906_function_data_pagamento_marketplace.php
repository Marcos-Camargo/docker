...
return new class extends CI_Migration
{
	...
	public function up() {
        $this->db->query('DROP FUNCTION IF EXISTS `data_pagamento_marketplace`');
        $this->db->query("CREATE FUNCTION `data_pagamento_marketplace`(id_pedido INT(10)) RETURNS VARCHAR(255) BEGIN 
...
DATE_FORMAT(OPD.data_pagamento_marketplace, '%d/%m/%Y')
...
IF(@checkDate = 0) THEN
INSERT INTO orders_payment_date (order_id, data_pagamento_marketplace) VALUE (id_pedido, STR_TO_DATE(@dataRetorno, '%d/%m/%Y'));
ELSE
UPDATE orders_payment_date SET data_pagamento_marketplace = STR_TO_DATE(@dataRetorno, '%d/%m/%Y') WHERE order_id = id_pedido;
END IF;
END IF;
