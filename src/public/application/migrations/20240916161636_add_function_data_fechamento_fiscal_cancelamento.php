...
return new class extends CI_Migration
{
	...
	public function up() {
		$this->db->query('DROP FUNCTION IF EXISTS `data_fechamento_fiscal_cancelamento`');

        $query = "CREATE FUNCTION `data_fechamento_fiscal_cancelamento`(id_pedido INT(10)) RETURNS varchar(255) CHARSET utf8mb4
					BEGIN 
					...
					DATE_FORMAT(OPD.data_fechamento_fiscal_cancelamento, '%d/%m/%Y')
					...
					IF(@checkDate = 0) THEN
					INSERT INTO orders_payment_date (order_id, data_fechamento_fiscal_cancelamento) VALUE (id_pedido, STR_TO_DATE(@dataRetorno, '%d/%m/%Y'));
					ELSE
					UPDATE orders_payment_date SET data_fechamento_fiscal_cancelamento = STR_TO_DATE(@dataRetorno, '%d/%m/%Y') WHERE order_id = id_pedido;
					END IF;
					END IF;
