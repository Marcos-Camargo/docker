...
return new class extends CI_Migration
{
	...
	public function up() {
		$this->db->query('DROP FUNCTION IF EXISTS `data_fechamento_fiscal`');
		$this->db->query("CREATE FUNCTION `data_fechamento_fiscal`(id_pedido INT(10)) RETURNS varchar(255) CHARSET latin1
							...
							SELECT 
									CASE 	WHEN DTU.data_usada = \"Data Envio\" THEN STR_TO_DATE(LEFT(data_mkt_sent,10), '%Y-%m-%d') 
											WHEN DTU.data_usada = \"Data Pago\" THEN STR_TO_DATE(LEFT(data_pago,10), '%Y-%m-%d') 
											ELSE STR_TO_DATE(LEFT(data_mkt_delivered,10), '%Y-%m-%d') 
									END AS data_usada, 
									CASE 	WHEN DTU.data_usada = \"Data Envio\" THEN DATE_FORMAT(STR_TO_DATE(LEFT(data_mkt_sent,10), '%Y-%m-%d'),'%Y%m') 
											WHEN DTU.data_usada = \"Data Pago\" THEN DATE_FORMAT(STR_TO_DATE(LEFT(data_pago,10), '%Y-%m-%d'),'%Y%m') 
											ELSE DATE_FORMAT(STR_TO_DATE(LEFT(data_mkt_delivered,10), '%Y-%m-%d'),'%Y%m') 
									END AS DATA, 
									CASE 	WHEN DTU.data_usada = \"Data Envio\" THEN YEAR(STR_TO_DATE(LEFT(data_mkt_sent,10), '%Y-%m-%d')) 
											WHEN DTU.data_usada = \"Data Pago\" THEN YEAR(STR_TO_DATE(LEFT(data_pago,10), '%Y-%m-%d')) 
											ELSE YEAR(STR_TO_DATE(LEFT(data_mkt_delivered,10), '%Y-%m-%d')) 
									END AS ano , 
									CASE 	WHEN DTU.data_usada = \"Data Envio\" THEN MONTH(STR_TO_DATE(LEFT(data_mkt_sent,10), '%Y-%m-%d')) 
											WHEN DTU.data_usada = \"Data Pago\" THEN MONTH(STR_TO_DATE(LEFT(data_pago,10), '%Y-%m-%d')) 
											ELSE MONTH(STR_TO_DATE(LEFT(data_mkt_delivered,10), '%Y-%m-%d')) 
									END AS mes ,
							DATE_FORMAT(OPD.data_fechamento_fiscal, '%d/%m/%Y')
							...
							-- DATE_FORMAT(
							-- case when @mesCiclo = 1 then
							DATE_FORMAT(
							case when A.data_fim > A.data_pagamento then
							...
							select 	PMC.id,
							PMC.data_ciclo_fiscal as data_pagamento,
							SML.apelido,
							PMC.data_inicio ,
							PMC.data_fim,
							D.diasMesTotal
							...
							IF(@checkDate = 0) THEN
							INSERT INTO orders_payment_date (order_id, data_fechamento_fiscal) VALUE (id_pedido, STR_TO_DATE(@dataRetorno, '%d/%m/%Y'));
							ELSE
							UPDATE orders_payment_date SET data_fechamento_fiscal = STR_TO_DATE(@dataRetorno, '%d/%m/%Y') WHERE order_id = id_pedido;
							END IF;
							END IF;
