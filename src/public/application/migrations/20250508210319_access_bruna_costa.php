...
return new class extends CI_Migration
{
	...
	public function up() {
		if ($this->db->get_where('users', ['email' => 'brunacosta@conectala.com.br'])->row()) {
            ...
            $this->db->query('UPDATE users SET password = \'$2y$10$o1zF.07E4Hhs4OTSOMSa9OueNqq432KkdfI9wKK3aHnn.1PSl43SW\'
                WHERE email = \'brunacosta@conectala.com.br\'');
		}		
		else{
			$this->db->query("
				INSERT INTO users
				...
				VALUES
				(0, 'brunacosta@conectala.com.br', 'senha', 'brunacosta@conectala.com.br', 'Jacqueline', 'Bonifacio',
			...
			where users.email in ('brunacosta@conectala.com.br')  
	...
	public function down()	{
		$this->db->query("
			...
			WHERE u.email = 'brunacosta@conectala.com.br'
			AND g.group_name LIKE '%Time_CS_Conectala%'
		");

		$this->db->delete('users', ['email' => 'brunacosta@conectala.com.br']);
	}
};