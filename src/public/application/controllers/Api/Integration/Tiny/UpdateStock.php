...
class UpdateStock extends REST_Controller
{
    ...
    public function index_post()
    {
        ...
        $product = json_decode(file_get_contents('php://input'));
        //$this->log_data('WebHook', 'WebHookUpdateProd - Start', json_encode($product), "I");

        // chegou um tipo diferente de estoque (não implementado)
        if ($product->tipo != "estoque") {
            //$this->log_data('WebHook', 'UpdateStock/index_post', "Chegou um tipo de atualização diferente de estoque. Chegou={$product->tipo} - payload=" . json_encode($product) , "W");
            $this->log_integration("Recebimento Indevido de Atualização de Estoque", "Chegou um tipo de atualização diferente de estoque. Chegou={$product->tipo}", "E");
            ob_clean();
        ...
        // Inicia transação
        $this->db->trans_begin();
        $this->setUniqueId($product->dados->idProduto); // define novo unique_id

        $update = $this->product->updateStock($product->dados);
        ...
        if ($update !== false) {
            ...
            if ($update === null) {
                $this->log_integration("Foi tentado atualizar o estoque/variação do produto {$product->dados->skuMapeamento}", "<h4>Não foi encontrado o produto/variação na lista de preço</h4> <strong>SKU</strong>: {$product->dados->skuMapeamento}", "E");
            } else {
                $tipoVariacao = $product->dados->skuMapeamentoPai == "" ? "N" : "V";

                if($tipoVariacao == "N")
                    $this->log_integration("Estoque do produto {$product->dados->skuMapeamento} atualizado", "<h4>O estoque do produto {$product->dados->skuMapeamento} foi atualizado com sucesso.</h4><strong>Estoque alterado:</strong> {$product->dados->saldo}", "S");
                if($tipoVariacao == "V")
                    $this->log_integration("Estoque da variação {$product->dados->skuMapeamento} atualizada", "<h4>O estoque do produto {$product->dados->skuMapeamento} foi atualizado com sucesso.</h4><strong>Estoque alterado:</strong> {$product->dados->saldo}", "S");
            }
