...
class FreteCarrefour extends FreteConectala {
     ...
     * Example request
     *
     * /Apicar/freight?sku=Q_CAR_1|3,Q_CAR_2|1&shipping_zip_code=88010000
     *
        ...
        if ($tempValue !== null && $tempDeadLine !== null) {
            foreach ($items as $iten) {
                array_push($freights, array(
                    'sku_shop' => $iten['sku'],
                    'quantity' => (int)$iten['qty'],
                    'offer_quantity' => (int)$quote['data']['skus'][$iten['sku']]['current_qty'],
                    'shipping_price' => $tempValue,
                    'delivery_time' => $tempDeadLine,
                    'shipping_type_code' => 'normal',
