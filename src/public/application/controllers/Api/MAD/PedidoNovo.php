...
class PedidoNovo extends REST_Controller {
    ...
    public function index_post()
    {
		...
		if (is_null($data)) {
			$this->log_data('api', 'MAD_PedidoNovo', 'ERRO - Dados com formato errado recebidos do Madeira Madeira='.$dataPost,'E');
			$this->response([
		...
		$this->log_data('api', 'MAD_PedidoNovo', 'Dados Recebidos:'.print_r($dataPost,true),'I');
		...
		// Por enquanto não faço nada com esta informação 
	    $this->log_data('api', 'MAD_PedidoNovo', 'Pedido Numero Marketplace '.$data['order'].' novo no Madeira Madeira','I');
