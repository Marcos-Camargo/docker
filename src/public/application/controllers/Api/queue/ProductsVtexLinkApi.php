...
class ProductsVtexLinkApi extends ProductsConectala {
	...
	function insertProductVariant($variant=null)
	{
				...
				if ($atributoCat) {
					...
					if ($field_id!=0) {
                        ...
                        if (((int)$atributoCat['obrigatorio']) == 1
                            && ((int)$atributoCat['atributo_variacao']) == 1
                        ) { // mas é obrigatório e de variação
		...
		foreach($atributosCat as $atributoCat) {
			...
			if (!$atributo_prd) {  // o atributo não está preenchido 
                if (((int)$atributoCat['obrigatorio']) == 1
                    && ((int)$atributoCat['atributo_variacao']) == 0
	...
	private function changeSkuSpecifications($prd_to_integration, $variant)
	{
		...
		return true;  // removido para evitar problemas na Vtex de mudar as variações no marketplace 
