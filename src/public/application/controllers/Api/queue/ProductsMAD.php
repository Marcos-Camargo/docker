...
class ProductsMAD extends ProductsConectala {
	...
	function insertProductVariant($sku, $variant = null)
	{
		...
		if ($this->responseCode == 422) { // Deu um erro 
			...
			$retorno= $this->processURL($url, 'PUT',  $produto, $this->prd['id'], $this->int_to, 'Altera produto');
		}
	...
	function updateProductVariant($variant = null,$status_int =2)
	{
		...
		$produto = array(
			array (
				'sku' 			=> $sku,
		        'preco_de' 		=> (double)$price,
		        'preco_por' 	=> (double)$promotional_price,
		...
		if (is_null($variant)) {
			$this->updateMADLastPost($this->prd, null);
		}
		else {
			$prd = $this->prd;
			$prd['sku'] = $variant['sku'];
			...
			$this->updateMADLastPost($prd, $variant);
		}
	...
	function inactiveMKT() {
		...
		if ($this->prd['has_variants'] !== '') {
			foreach($this->variants as $key => $variant) {
				...
				echo 'Zerando estoque da variacao '.$variant['variant'].' sku '.$int['skubling']."\n";
				$this->zeroStock($int['skubling'], $int);
			}
		}
		else {
			$int = $this->model_integrations->getIntegrationsProductWithVariant($this->prd['id'], $this->int_to,null);
			echo 'Zerando estoque do produto sku '.$int['skubling']."\n";
			$this->zeroStock($int['skubling'], $int);
		}		
	}
	...
	function zeroStock($sku, $int) {
		...
		if (is_null($sku)) {
			echo "SKU ainda não preenchido \n";
			return true; 
		}
		if (!$this->getProductPublished($sku)) {
			echo "Não achei para zerar o estoque \n";
			return true; 
		}
		...
		if ($this->responseCode == 422) {
			echo " Já está ruim, e não vai aceitar mudar \n";
			echo " Erro URL: ". $url. " httpcode=".$this->responseCode."\n"; 
			echo " RESPOSTA ".$this->int_to.": ".print_r($this->result,true)." \n"; 
			echo " Dados enviados: ".print_r($produto,true)." \n"; 
			$this->log_data('batch',$log_name, 'ERRO no post produto site:'.$url.' - httpcode: '.$this->responseCode." RESPOSTA ".$this->int_to.": ".print_r($this->result,true).' DADOS ENVIADOS:'.print_r($prd_json,true),"W");
			return true;
		} 
		if (($this->responseCode != 200) && ($this->responseCode != 201) && ($this->responseCode != 202) && ($this->responseCode != 204)) { // Deu um erro 
			echo " Erro URL: ". $url. " httpcode=".$this->responseCode."\n"; 
			echo " RESPOSTA ".$this->int_to.": ".print_r($this->result,true)." \n"; 
			echo " Dados enviados: ".print_r($produto,true)." \n"; 
			$this->log_data('batch',$log_name, 'ERRO no post produto site:'.$url.' - httpcode: '.$this->responseCode." RESPOSTA ".$this->int_to.": ".print_r($this->result,true).' DADOS ENVIADOS:'.print_r($prd_json,true),"E");
	...
	function prepareProduct($sku) {
		...
		// $this->prd['description'] = substr(htmlspecialchars(strip_tags(str_replace("&nbsp;",' ',str_replace("</p>","\n",str_replace("<br>","\n",$this->prd['description'])))), ENT_QUOTES, "utf-8"),0,10000);;
//		$description = substr(htmlspecialchars(strip_tags($this->prd['description'],'<br>'), ENT_QUOTES, "utf-8"),0,10000);
        $description = strip_tags($this->prd['description']);
        $description = $this->RemoveCaracters($description);
    ...
    public function RemoveCaracters($description)
    {
        $inputs = ['&amp;','amp;','/gt;',' br ','&amp;','&nbsp;','nbsp;','br&lt;','br&gt;','&lt;br>','&amp;lt;','/&amp;gt;','    '];

        $descricao = str_replace($inputs, '', $description);
	...
	function montaArray($sku, $skupai, $variant = null) 
	{
		...
		if (count($images) > 20) { // limite de 20 imagens 
			array_splice($images, 20);
		}
    ...
    function updateMADLastPost($prd, $variant = null) 
	{
		$log_name =$this->router->fetch_class().'/'.__FUNCTION__;
		...
		$savedUltEnvio =$this->model_mad_last_post->createIfNotExist($this->int_to,$prd['id'], $variant_num, $data); 
		if (!$savedUltEnvio) {
