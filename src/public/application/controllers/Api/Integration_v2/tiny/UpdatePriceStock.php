...
class UpdatePriceStock extends REST_Controller
{
    /**
     * @var ToolsProduct
     */
    ...
    public function index_post()
    {
        ...
        */
        $precoPromocional = property_exists($product, 'precoPromocional') ? (float)$product->precoPromocional : 
                (property_exists($product, 'preco_promocional') ? (float)$product->preco_promocional : null);
        $precoPromocional = $precoPromocional > 0 ? $precoPromocional : null;
        $price            = empty($precoPromocional ?? null) ? (isset($product->preco) ? (float)$product->preco : 0) : $precoPromocional;
        $listPrice        = isset($product->preco) ? ((float)($product->preco > 0 ? $product->preco : $precoPromocional)) : $precoPromocional;
