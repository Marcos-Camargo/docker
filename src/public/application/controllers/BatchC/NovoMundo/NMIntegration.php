...
Realiza a integração na Novo Mundo

*/   
require 'NMAttributes.php';
...
{    
    ...
    function getNewOrders($api_keys)
    {
        $url = $this->url_api . "Orders?status=1,3";
        $response = $this->getRequest($url, $api_keys);
        return $response;
    }
    ...
    {
        $url = $this->url_api . "Orders/".  $order["bill_no"] ."/trackings/sent";
    ...
    {
        $url = $this->url_api . "Orders/". $order["bill_no"] ."/trackings/delivered";
        $response = $this->postRequest($url, $api_keys, $this->castSentOrder($order, $items, $nfe, $freight));
        return $response;
    }
	
	function nfesOrder($api_keys, $nfes_orders)
    {
        $url = $this->url_api . "Orders/nfe"; 
        $response = $this->postRequest($url, $api_keys, $nfes_orders);
        return $response;
    }
	
	function sentOrderNew($api_keys, $order, $data)
    {
        $url = $this->url_api . "Orders/".  $order["bill_no"] ."/shipped";
        $response = $this->putRequest($url, $api_keys, $data);
        return $response;
    }
	
	function cancelOrder($api_keys, $order, $data)
    {
        $url = $this->url_api . "Orders/".  $order["bill_no"] ."/canceled";
        $response = $this->putRequest($url, $api_keys, $data);
        return $response;
    }
    ...
    function getOrdersCanceled($api_keys)
    {
		$initialDate = date("Y-m-d",time() - 60 * 60 * 24* 15);
        $finalDate = date("Y-m-d",time());

        $url = $this->url_api.'Orders?status=96&start_date='.$initialDate;
        $url = $this->url_api.'Orders?status=96';
        $response = $this->getRequest($url, $api_keys);
        return $response;
    }
    ...
    function getAttributesNM($api_keys = false, $category_id = false, $into_to_SC = false)
    {
    ...
    function getAttributes($attributes_conectala, $attributes_nm)
    {
        $nmAttributes = new NMAttributes();

        return $nmAttributes->linkAttributes($attributes_conectala, $attributes_nm);        
    }


    function getVariants($variants_conectala, $attributes_nm)
    {
        $nmAttributes = new NMAttributes();

        return $nmAttributes->linkVariants($variants_conectala, $attributes_nm);
    }
    ...
    {
        $nmAttributes = new NMAttributes();
        return $nmAttributes->hasVariant(strtoupper($variant), $attributes);
    }
    ...
    public function castSentOrder($order, $items, $nfe, $freight) 
    {
        ...
        $body = array(
            ...
            'url' => $carrier_url, // 'https://www2.correios.com.br/sistemas/rastreamento/',
            'carrier' => $carrier,
            'invoice' => array(
                'number' => $nfe['nfe_num'],
                'serie' => $nfe['nfe_serie'],
