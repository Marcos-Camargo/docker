...
class ViaOrders extends BatchBackground_Controller {
	...
	function cancelar($id) 
	{
		...
		$response = $this->integration->getOrder($authorization, $id);

		if ($response['httpcode'] < 300) 
		{
			$order = json_decode($response['content'], true);
			$response_cancel = $this->cancelOrder($authorization, $order);
		}
	}
	...
	function approved($id = null)
	{
		...
		$this->syncApproveds($authorization);
	}
	...
	function updateFrete($order_id) 
	{
		...
		$order = $this->integration->getOrder($authorization, $order_id);
	...
	function syncFrete($order)
	{
        if ($order_exist = $this->model_orders->getOrdersDatabyNumeroMarketplace($order['id'])) {
            $this->calculofrete->updateShipCompanyPreview($order_exist['id']);
        }
        ...
        if ($store['freight_seller'] == 1 && in_array($store['freight_seller_type'], [3, 4])) { // intelipost
			...
			if ($order_exist = $this->model_orders->getOrdersDatabyNumeroMarketplace($order['id'])) {
                ...
                if ($precode === false)
                    $this->model_orders->setShipCompanyPreview($order_exist["id"], 'Precode', 'Precode', 10);
                else
                    $this->model_orders->setShipCompanyPreview($order_exist["id"], $precode['provider'], 'Precode', $precode['deadline']);
			}
		}
		else {
			if ($todos_correios) {
    ...
    function syncOrders()
    {
		...
		$this->syncNews($authorization);
		$this->syncApproveds($authorization);
		$this->syncCancelled($authorization);
    }

    function syncNews($authorization, $offset = 0, $limit = 50) 
    {
		$log_name =$this->router->fetch_class().'/'.__FUNCTION__;
		...
		$response = $this->integration->getOrdersNew($authorization, $initialDate, $offset, $limit);

		if ($response['httpcode'] != 200) {
			echo "httpcode=".$response['httpcode']."\n";
			echo " RESPOSTA ".$this->getInt_to().": ".print_r($response,true)." \n"; 
			$this->log_data('batch',$log_name, 'ERRO no get - httpcode: '.$response['httpcode'].' RESPOSTA: '.print_r($response,true),"E");
			return;
		}
		
		$response = json_decode($response['content'], true);
        
        foreach ($response["orders"] as $orderMkt) {
			$this->newOrder($orderMkt);
		}	

		if (count($response["orders"]) >= $limit) {
			$_offset += $limit;
			$this->syncNews($authorization, $_offset);
		}
	}
	...
	private function newOrder($orderMkt)
	{
		...
		foreach($orderMkt['items'] as $item) {
			...
			if (empty($prf)) {
				...
				if (empty($prf))  {
					echo 'O pedido '. $orderMkt['id'] .' possui produto '.$sku_item.' que não é do Marketplace '.$this->getInt_to()."! Ordem não importada"."\n";
					$this->log_data('batch',$log_name,'O pedido '.$orderMkt['id'].' possui produto '.$sku_item.' que não é do Marketplace '.$this->getInt_to()."! Ordem não importada","E");
					$erro = true; 
		...
		// campos que não tem no via varejo
		$clients['ie'] = '';
		$clients['rg'] = '';

		$useShippingAddress = true;
		if (!isset($orderMkt['shipping'])) $useShippingAddress = false;

		$orders['customer_phone']           = $phone_1;
		$orders['customer_address']         = $orderMkt[$useShippingAddress ? 'shipping' : 'billing']['address'];
		$orders['customer_address_compl']   = $orderMkt[$useShippingAddress ? 'shipping' : 'billing']['complement'];
		$orders['customer_address_neigh']   = $orderMkt[$useShippingAddress ? 'shipping' : 'billing']['quarter'];
		$orders['customer_address_num']     = $orderMkt[$useShippingAddress ? 'shipping' : 'billing']['number'];
		$orders['customer_address_zip']     = preg_replace("/[^0-9]/", "", $orderMkt[$useShippingAddress ? 'shipping' : 'billing']['zipCode']);
		$orders['customer_address_uf']      = $orderMkt[$useShippingAddress ? 'shipping' : 'billing']['state'];
		$orders['customer_address_city']    = $orderMkt[$useShippingAddress ? 'shipping' : 'billing']['city'];
		...
		if ($client_id = $this->model_clients->getByOrigin($this->getInt_to(),$orderMkt['customer']['documentNumber'])) {
		...
		foreach($orderMkt['items'] as $item) {
			...
			if ($prd['is_kit'] ==0) {
				...
				if ($prd['has_variants'] != '') {
                    ...
                    $var = $this->model_products->getVariants($prd['id'],$variant);
                    if ($var && $var['sku'] != '')
                        $items['sku'] = $var['sku'];
				}
                ...
                $items['discount'] = isset($var) && $var && !empty($var['price']) ? ($var['price'] - $item['salePrice']) : ($prd['price'] - $item['salePrice']);
				...
				if (!$cancelar_pedido) {
					...
					$this->model_blingultenvio->reduzEstoque($this->getInt_to(),$prd['id'],$items['qty']);
				...
				if (!$cancelar_pedido) {
					$this->model_blingultenvio->reduzEstoque($this->getInt_to(),$prd['id'], $items['qty']);  // reduzo o estoque do produto KIT no Bling_utl_envio
				}
				//verificacao do frete 
				$todos_tipo_volume = $todos_tipo_volume && $this->calculofrete->verificaTipoVolume($prf,$origem['state'],$destino['state']); 
				if ($todos_tipo_volume) { // se é tipo_volume não pode ser correios e não procisa consultar os correios
        ...
        $this->log_data('batch',$log_name,"Pedido {$orderMkt['id']} incluído\n\n".json_encode($orderMkt));
	...
	private function markApproved($authorization, $order) 
	{
		...
		if ($order_exist = $this->model_orders->getOrdersDatabyNumeroMarketplace($order['id'])) 
		{
			...
			if ($order_exist['paid_status'] == 1)
			{
				...
				$query = $this->db->query($sql, array($order['id'], $this->getInt_to()));
	...
	private function markCancelled($authorization, $order)
	{
		...
		if ($order_exist = $this->model_orders->getOrdersDatabyBill($this->getInt_to(), $order['id'])) {
