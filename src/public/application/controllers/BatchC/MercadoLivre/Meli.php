...
class Meli {
     ...
     * @return string
     */
    public function getAuthUrl($redirect_uri, $auth_url, $state = null) {
    ...
    public function authorize($code, $redirect_uri) {
		...
		while($http429) {
			...
			if (($request["httpCode"] == 429) || ($request["httpCode"] == 504)) {
				echo "Estourou o limite ".$request["httpCode"]."\n";
				sleep(15);
			}
			else {
				$http429=false;
    ...
    public function get($path, $params = null, $assoc = false) {
		...
		$http429=true;
		while($http429) {
		  	$request = $this->execute($path, $opts, $params, $assoc);
			if (($request["httpCode"] == 429) || ($request["httpCode"] == 504)) {
				echo "Estourou o limite ".$request["httpCode"]."\n";
				sleep(15);
			}
    ...
    public function post($path, $body = null, $params = array()) {
       ...
       // var_dump($body);
        ...
        // https://developers-forum.mercadolibre.com/topic/2420-erro-ao-enviar-descri%C3%A7%C3%A3o-que-contenha-o-caractere/#comment-10624
		...
		$http429=true;
		while($http429) {
		  	$request = $this->execute($path, $opts, $params);
			if (($request["httpCode"] == 429) || ($request["httpCode"] == 504)) {
				echo "Estourou o limite ".$request["httpCode"]."\n";
				sleep(15);
			}
