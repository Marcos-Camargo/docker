...
class MLOrdersStatusHub extends BatchBackground_Controller {
	...
	function mandaNfe()
	{
		...
		foreach ($ordens_andamento as $order) {
			...
			// $msg = 'Prezado(a) '.$order['customer_name'].',<br><br>A Nota Fiscal Eletrônica (NF-e) de seu pedido feito no Mercado Livre foi gerada com sucesso!<br>Lembramos que, junto com seu pedido, você receberá uma representação simplificada da Nota Fiscal Eletrônica chamada DANFE (Documento Auxiliar da Nota Fiscal Eletrônica) de número '.$nfe['nfe_num'].'<br><br>Caso queira consultar a versão XML da nota, basta acessar o link http://www.nfe.fazenda.gov.br/portal/consulta.aspx e inserir a chave da NF.<br><br>Chave da NF: '.$nfe['chave'].'<br><br>CASO VOCÊ PRECISE DE ALGUM ESCLARECIMENTO OU NECESSIDADE DE SOLUÇÃO EM RELAÇÃO AO SEU PEDIDO, ESTE É O NOSSO CANAL DE COMUNICAÇÃO MAIS RÁPIDO. BASTA NOS ENVIAR UMA MENSAGEM POR AQUI QUE IREMOS ATENDÊ-LO (DE SEGUNDA A SEXTA-FEIRA, DAS 9:00H ÀS 18:00H)! =)<br><br>Obrigado pela confiança!<br>'.$this->equipe;
	...
	function mandaEnviado()
	{
		...
		foreach ($ordens_andamento as $order) {
			...
			$resumo_itens = '';
			$endereco_entrega =$order['customer_address'].', '.$order['customer_address_num'];
			...
			/*
			$msg = 'Prezado(a) '.$order['customer_name'].', 
			<br><br>
			Informamos que seu pedido nº '.$order['numero_marketplace'].', foi enviado pra transportadora e logo chegará ao seu destino :)
			<br>
			A entrega será realizada de acordo com as informações abaixo:
			<br><br>
			'.$resumo_itens.'
			<br><br>
			Endereço que realizaremos a entrega: 
			<br>
			Código de rastreio: '.$endereco_entrega.'
			<br><br>
			Sua consulta estará disponível em até 48h após o recebimento desta mensagem.
			<br><br>
			CASO VOCÊ PRECISE DE ALGUM ESCLARECIMENTO OU NECESSIDADE DE SOLUÇÃO EM RELAÇÃO AO SEU PEDIDO, ESTE É O NOSSO CANAL DE COMUNICAÇÃO MAIS RÁPIDO. BASTA NOS ENVIAR UMA MENSAGEM POR AQUI QUE IREMOS ATENDÊ-LO (DE SEGUNDA A SEXTA-FEIRA, DAS 9:00H ÀS 18:00H)! =)
			<br><br>
	...
	function mandaEntregue()
	{
		...
		foreach ($ordens_andamento as $order) {
			...
			// $msg = 'Olá '.$order['customer_name'].',<br><br>Seu pedido consta entregue pela transportadora. Você confirma a informação? Caso positivo, conte-nos como está sendo sua experiência com o produto. Atendeu às suas necessidades? Se possível, poderia fazer a avaliação no Mercado Livre? Nos ajudaria bastante!<br><br>Agradecemos a confiança e esperamos lhe atender mais vezes no futuro!<br>'.$this->equipe;
			if ($this->sendMessage($order,$msg)) {
				// Avisado que foi entregue na transportadora 
