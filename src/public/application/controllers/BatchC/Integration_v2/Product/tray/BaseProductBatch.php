...
use Integration_v2\tray\Services\ProductCatalogService;
...
abstract class BaseProductBatch extends BatchBackground_Controller
{
    ...
    protected function import(): bool
    {
        ...
        $countException = 1;
        $processed = 0;
        $filter = !empty($this->productSku) ? ['reference' => $this->productSku] : $this->getProductPaginationFilters();
        try {
            while (true) {
                echo "PAGE: {$page}\n";
                try {
                    $productList = $this->toolsProduct->getProductsPagination($filter, $page, $perPage);
                    echo "TOTAL: {$productList->paging->total}\n";
                    ...
                    $remain = $productList->paging->total - $processed;
