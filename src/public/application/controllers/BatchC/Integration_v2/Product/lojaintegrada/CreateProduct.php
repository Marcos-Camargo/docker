 ...
 * Class CreateProduct
 *
 * php index.php BatchC/Integration_v2/Product/lojaintegrada/CreateProduct run {ID} {STORE}
 *
 */
...
class CreateProduct extends BatchBackground_Controller
{
    ...
    public function getProductsToCreate(): bool
    {
        $page = 1;
        $countException = 1;
        $filter['createdAt'] = $this->toolsProduct->dateLastJob ?? date('Y-m-d H:i:s', strtotime('-50 years'));
        try {
            while (true) {
                echo "PAGE: {$page}\n";
                try {
