...
require_once APPPATH . "libraries/Integration_v2/magalu/ToolsProduct.php";
...
use Integration_v2\magalu\Services\BaseProductImportService;
...
abstract class BaseProductImport extends BatchBackground_Controller
{
    ...
    public function import(): bool
    {
        ...
        while (true) {
            try {
                echo "[PROCESS][LINE:" . __LINE__ . "] OBTENDO LISTAGEM DE PRODUTOS MAGALU. PAGINA: {$page}\n";
                $result = $this->toolsProduct->getProductsPagination([], $page, $size);
                if (!$result){
                    break;
                }
                try {
                    $this->productImportService->handleWithRawList($result ?? []);
                } catch (Throwable $e) {
                    echo "[ERROR][LINE:" . __LINE__ . "] PROCESSAMENTO LISTAGEM DE PRODUTOS MAGALU. PAGINA: {$page} - {$e->getMessage()}\n";
                }
            } catch (Throwable $e) {
                echo "[ERROR][LINE:" . __LINE__ . "] CONSULTA LISTAGEM DE PRODUTOS MAGALU. PAGINA: {$page} - {$e->getMessage()}\n";
                $this->toolsProduct->log_integration(
