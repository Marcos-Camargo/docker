 ...
 * Class UpdatePrice
 *
 * php index.php BatchC/Integration_v2/Product/hub2b/UpdatePriceStock run {ID} {STORE}
 *
 */

require APPPATH . "libraries/Integration_v2/hub2b/ToolsProduct.php";
...
class UpdatePriceStock extends BatchBackground_Controller
{
                ...
                if (!empty($productDB['has_variants'])) {
                    ...
                    foreach ($variations as $variation) {
                        $skus[] = [
                            ...
                            'uniqueId'      => "{$productDB['product_id_erp']}:{$this->toolProduct->store}",
                        ];
                    }
    ...
    private function updatePriceStockProduct(array $skus)
    {
        foreach ($skus as $key_sku => $sku) {
            $this->toolProduct->setUniqueId($sku['uniqueId']);
            ...
            list($intStock, $intPrice) = $this->toolProduct->fetchProductPriceStockProductVariation($sku['sku'], $sku['varSku'] ?? null);
            if (isset($intStock['stock']) && !empty($intStock['stock'])) {
