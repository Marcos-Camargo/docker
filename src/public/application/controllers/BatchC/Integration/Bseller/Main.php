...
class Main extends Integration
{
    ...
    {
        parent::__construct();
        $this->setTypeIntegration('bseller');
    }
    ...
    public function setDataIntegration($store_id)
    {
        ...
        $this->setToken($credentials->token_bseller);
        $this->setInterface($credentials->interface);

        if ($this->validateToken() === false) {
            $this->shutAppStatus = true;
            $this->shutAppTitle = "Erro Bseller";
            
        } elseif ($this->validateToken() === null) {
            $this->shutAppStatus = false;
            $this->shutAppTitle = "Servidor Bseller sem resposta";
            $this->shutAppDesc = "Não foi possível realizar a consulta ao servidor da BSeller, em breve tentaremos novamente!";
        } 
