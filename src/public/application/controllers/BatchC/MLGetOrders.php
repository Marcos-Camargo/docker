...
class MLGetOrders extends BatchBackground_Controller {
    ...
    function getorders()
    {
		...
		while (true) {
			...
			foreach ($results as $pedido) {
				...
				echo "----------------------------------------".$pedido['id']."\n";
				if ($pedido['id'] == '4543231311')  { //O pedido veio sem nada. Peguei estas informações direto no Mercado livre
					echo "Aqui \n";
					$clients['email']='';  
					
					$clients['customer_name'] = 'jeronimo da Silva';
					$orders['customer_name'] = $clients['customer_name'];	
				}
					...
					if (is_null($prf)) {
						...
						if (is_null($prf)) {
							if (strrpos($skubling, "-") !=0) {
								$sku_item = substr($skubling, 0, strrpos($skubling, "-"));
								$sql = "SELECT * FROM bling_ult_envio WHERE skubling = ? AND (int_to = ? OR int_to = ?)";
								$query = $this->db->query($sql, array($sku_item,$this->getInt_to(),'MLC'));
								$prf = $query->row_array();
							}
				...
				$this->log_data('batch',$log_name,"Pedido {$pedido['id']} {$typeMessageLog}\n\n".json_encode($pedido));
