...
*/   
class CarGetOneOrder extends BatchBackground_Controller {
	...
	function run($id=null)
	{
		...
		$this->getkeys(1,0);
		if (is_null($id)) {
			echo "informe o código do pedido\n";
		}
		else {
			$this->getorder($id);
		}
    ...
    function getorder($id)
    {
		...
		$url = 'https://'.$this->getSite().'/api/orders/'.$id;
		echo 'url='.$url."\n";
		...
		if ($retorno['httpcode'] == 404) {
			echo "Pedido ".$id." não existe no Carrefour\n";
			return;
		}
		...
		foreach($pedido['order_lines'] as $item) {
			...
			if (empty($prf)) {
				if (strrpos($sku_item, "-") !=0) {
					$sku_item = substr($item['offer_sku'], 0, strrpos($item['offer_sku'], "-"));
					$sql = "SELECT * FROM bling_ult_envio WHERE skubling = ? AND int_to = ?";
					$query = $this->db->query($sql, array($sku_item,$this->getInt_to()));
					$prf = $query->row_array();
				}
		...
		if ( array_key_exists('order_additional_fields',$pedido)) {
			...
			// Segundo a carredour usar sempre o Order_Additional_fields, mas se não existe, 
			if (!is_null($pedido['customer']['shipping_address'])) {
				...
				$orders['customer_name'] = $pedido['customer']['shipping_address']['firstname'].' '.$pedido['customer']['shipping_address']['lastname'];
			}
		}
		

		/** Usar sempre o Other_aditional_fields
		if (!is_null($pedido['customer']['billing_address'])) {
			...
			$clients['country'] = $pedido['customer']['billing_address']['country'];
		...
		// $orders['date_time'] = $pedido['created_date'];
		...
		if ( array_key_exists('promotions',$pedido)) {
		...
		foreach($pedido['order_lines'] as $item) {
			$skubling = $item['offer_sku'];
			$sql = "SELECT * FROM bling_ult_envio WHERE skubling = ? AND int_to = ?";
			$query = $this->db->query($sql, array($skubling, $this->getInt_to()));
			$prf = $query->row_array();
			if (empty($prf)) {  // deve ser um item com variação.
				if (strrpos($skubling, "-") !=0) {
					$skubling = substr($item['offer_sku'], 0, strrpos($item['offer_sku'], "-"));
					$sql = "SELECT * FROM bling_ult_envio WHERE skubling = ? AND int_to = ?";
					$query = $this->db->query($sql, array($skubling,$this->getInt_to()));
					$prf = $query->row_array();
				}
			...
			if ($prd['is_kit'] ==0) {
				...
				$items['rate'] = $item['price_unit'];
				$items['amount'] = (float)$item['price_unit'] * (float)$item['quantity'];
				
				$items['discount'] = (float)$item['price'] - $items['amount']; 
				...
				if (!$cancelar_pedido) {
					$this->model_blingultenvio->reduzEstoque($this->getInt_to(),$prd['id'],$item['quantity']);  // reduzo o estoque do produto KIT no Bling_utl_envio
				}
			}
		...
		if ($store['freight_seller'] == 1 && in_array($store['freight_seller_type'], [3,4,5,6])) { // frete externo
			...
			switch ($store['freight_seller_type']) {
				case 3:
					$tipoFreteExterno = 'intelipost_seller';
					break;
				case 4:
					$tipoFreteExterno = 'intelipost_sellercenter';
					break;
				case 5:
					$tipoFreteExterno = 'freterapido_seller';
					break;
				case 6:
					$tipoFreteExterno = 'freterapido_sellercenter';
					break;
			}

			switch ($tipoFreteExterno) {
				case 'intelipost_seller':
				case 'freterapido_seller':
					$tokenExterno = $store['freight_seller_end_point'];
					break;
				case 'intelipost_sellercenter':
					...
					$tokenExterno = $rowSettings['value'];
					break;
				case 'freterapido_sellercenter':
					$querySettings   = $this->db->query('SELECT * FROM settings WHERE name = ?', array('token_freterapido_sellercenter'));
					$rowSettings     = $querySettings->row_array();
					$tokenExterno = $rowSettings['value'];
					break;
				default:
			...
			switch ($tipoFreteExterno) {
				case 'intelipost_seller':
				case 'intelipost_sellercenter':
					$responsePriceDeadline = $this->calculofrete->getPriceAndDeadlineIntelipost($fr, $cross_docking, $orders['customer_address_zip'], $tokenExterno);
					break;
				case 'freterapido_seller':
				case 'freterapido_sellercenter':
					$responsePriceDeadline = $this->calculofrete->getPriceAndDeadlineIntelipost($fr, $cross_docking, $orders['customer_address_zip'], $tokenExterno);
			...
			if ($responsePriceDeadline !== false)
				$this->model_orders->setShipCompanyPreview($order_id, 'Intelipost', 'Intelipost', $responsePriceDeadline['deadline']);
		}
