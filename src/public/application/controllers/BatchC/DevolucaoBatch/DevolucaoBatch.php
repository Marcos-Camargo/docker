...
/**
 * Class DevolucaoBatch
 */
class DevolucaoBatch extends GenericBatch
{
    ...
    public function runDevolutions($id = null, $params = null): void
    {
        ...
        if($produtos){
            foreach($produtos as $key => $produto){
                ...
                if($frete == 'sellercenter'){

                    // INTEGRAL E DENTRO DO CICLO - 426
                    if($ciclo && $devolucao == 'integral')
                    {
                        ...
                        if($regra->rule_full_refund_inside_cicle == 'no_charge'){
                            // valor_liquido - frete
                            $repasse = ($produto->gross_amount - $produto->service_charge) - $produto->total_ship;
                        }

                        // ESTORNO DA COMISSAO DENTRO DO CICLO
                        if($regra->rule_full_refund_inside_cicle == 'commission_reversal'){
                            $valor_comissao_frete = $produto->total_ship * ($produto->service_charge_freight_value / 100);
                            // valor_bruto_produtos + comissao_frete
                            $repasse = $produto->total_order + $valor_comissao_frete;
                        }
                    }
                    else
                    // INTEGRAL E FORA DO CICLO - 427
                    if(!$ciclo && $devolucao == 'integral'){
                            ...
                            // VALOR LIQUIDO
                            if($regra->rule_full_refund_outside_cicle == 'reversal_net_order_value'){
                                // Estorno do valor líquido do pedido (produto + frete - comissão produto - comissão frete)
                                // valor_liquido - frete
                                $repasse = (($produto->total_order + $produto->total_ship) - $produto->service_charge) - $produto->total_ship;
                            }
                        }
                    else
                    // PARCIAL E DENTRO DO CICLO - 428
                    if($ciclo && $devolucao == 'parcial')
                            {

                                // SEM COBRANÇA
                                if($regra->rule_partial_refund_inside_cicle == 'no_charge'){
                                    $n_produtos_pedido = $qtdProductsInOrder;
                                    ...
                                    $repasse = ($valor_produto - $comissao_produto_devolvido) - (($valor_comissao_frete) / $n_produtos_pedido);

                                }

                                // Estorno da comissão do(s) produto(s) devolvido(s) + comissão do frete total
                                if($regra->rule_partial_refund_inside_cicle == 'refund_commission_returned_products_commission_total_shipping'){
                                    $valor_comissao_frete = $produto->total_ship * ($produto->service_charge_freight_value / 100);
                                    $valor_produto = $produto->return_total_value;
                                    // (valor_bruto_produto_devolvido) + (comissao_frete)
                                    $repasse = ($valor_produto) + ($valor_comissao_frete);
                                }

                                // Estorno da comissão do(s) produto(s) devolvido(s) + comissão do frete parcial
                                if($regra->rule_partial_refund_inside_cicle == 'refund_commission_returned_products_commission_partial_shipping'){
                                    // Cálculo estorno seller: (valor_bruto_produto_devolvido) + (comissao_frete/n_produtos_pedido)
                                    $valor_comissao_frete = $produto->total_ship ;
                                    $valor_produto = $produto->return_total_value;
                                    // (valor_bruto_produto_devolvido) + (comissao_frete/n_produtos_pedido)
                                    $repasse = $valor_produto + ($valor_comissao_frete / $qtdProductsInOrder);

                                }
                            }
                    else
                                // PARCIAL E FORA DO CICLO - 429
                                if(!$ciclo && $devolucao == 'parcial'){
                        ...
                        if($regra->rule_partial_refund_outside_cicle == 'refund_gross_value_returned_products_product_partial_freight'){
                            ...
                            $comissao_produto_devolvido = number_format($comissao_produto_devolvido/$qtdProductsInOrder, 2, '.', '.');
                                        //(valor_bruto_produto_devolvido - comissao_produto_devolvido) + (comissao_frete)
                                        $repasse = ($valor_produto - $comissao_produto_devolvido) + ($valor_comissao_frete);
                            ...
                            $comissao_produto_devolvido = ($produto->gross_amount - $produto->total_ship) * ($produto->service_charge_rate / 100);
                            $comissao_produto_devolvido = number_format($comissao_produto_devolvido/$qtdProductsInOrder, 2, '.', '.');
                                        // (valor_produto_devolvido - comissao_produto_devolvido) + (comissao_frete/n_produtos_pedido)
                                        $repasse = ($valor_produto - $comissao_produto_devolvido) + (($valor_comissao_frete)/$qtdProductsInOrder);
