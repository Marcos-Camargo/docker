...
class DB_ExportRegions extends BatchBackground_Controller
{   
    ...
    private function processExport($params)
    {
        ...
        if ($this->conta('regions', $db_ms) > 0) {
            echo "Tabelas regions já contém registro em Freight Tables. Não pode ser migrada novamente. Caso precisa migrar, deve limpar os dados.\n";
            return false;
        }
     
        $this->conta('regions', $this->db);
        // fim da parte nova

        $offset = 0;
        while (true) {
            $results = $this->db->select('idRegiao, Nome')
                ->order_by('idRegiao', 'ASC')
                ->limit(50000, $offset)
                ->get('regions')
                ->result_array();
            ...
            if (!empty($insert)) {
                $db_ms->insert_batch('regions', $insert);
            }
        }
        $this->conta('regions', $db_ms);
    }
