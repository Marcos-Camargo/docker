...
{   
    ...
    private function processExport($params)
    {
        ...
        if ($this->conta('integration_logistic', $this->db, "WHERE integration in ('sgpweb','intelipost','freterapido','sequoia','correios')") > 0) {
        ...
        while (true) {
            ...
            foreach ($results as $result) {
                $insert[] = array(
                    ...
                    'date_updated'      => $result['updated_at'] ?? dateNow()->format(DATETIME_INTERNATIONAL),
                    'date_created'      => $result['created_at'] ?? dateNow()->format(DATETIME_INTERNATIONAL),
        ...
        $this->conta('integration_logistic', $this->db, "WHERE integration in ('sgpweb','intelipost','freterapido','sequoia','correios')");
    }
