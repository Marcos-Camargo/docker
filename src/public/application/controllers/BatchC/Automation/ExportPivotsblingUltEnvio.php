...
class ExportPivotsblingUltEnvio extends BatchBackground_Controller
{   
    ...
    private function processExport()
    {
        ...
        while ($proceed) {
            ...
            foreach ($results as $result) {
                ...
                $sku_local            = addslashes($result["skubling"]);
                $product_id           = $result["prd_id"];
                $quantity             = $result["qty_atual"];
                $quantity_total       = $result["qty"];
                $height               = addslashes(str_replace(",", ".", preg_replace("/[^0-9.,]/", "", $result["altura"])));
                $width                = addslashes(str_replace(",", ".", preg_replace("/[^0-9.,]/", "", $result["largura"])));
                $length               = addslashes(str_replace(",", ".", preg_replace("/[^0-9.,]/", "", $result["profundidade"])));
                $gross_weight         = addslashes(str_replace(",", ".", preg_replace("/[^0-9.,]/", "", $result["peso_bruto"])));
                ...
                if (!is_null($result["data_ult_envio"]) && !empty($result["data_ult_envio"])) {
                    $created_at       = $result["data_ult_envio"];
                    $updated_at       = $result["data_ult_envio"];
                } else{
                ...
                $insert .= "('$marketplace', $company_id, $store_id, '$cnpj', NULL, '$zip_code', '$ean', '$sku_seller', '$sku_mkt', '$sku_local', $product_id, $product_variant, $price, $list_price, $category_id, $quantity, $quantity_total, $height, $width, $length, $gross_weight, $quantity_per_package, $crossdocking, '$created_at', '$updated_at')";
            }
            $insert .= ";";
            ...
            $dir = getcwd() . "/$dir";
            if (is_dir($dir)) {
                file_put_contents("$dir/ms_shipping_pivots_bling_ult_envio" . date("Ymd_His") . "_$offset-$iteration.sql", $insert);
            }
        }
    }
}
