...
class ExportPivotsVtexUltEnvio extends BatchBackground_Controller
{   
    ...
    private function processExport()
    {
        ...
        while ($proceed) {
            ...
            foreach ($results as $result) {
                ...
                $quantity_total       = $result["qty_atual"];
                ...
                $seller_id            = null;
                if (!is_null($result["seller_id"]) && !empty($result["seller_id"])) {
                ...
                $insert .= "('$marketplace', $company_id, $store_id, '$seller_id', '$cnpj', '$zip_code', '$ean', '$sku_seller', '$sku_mkt', NULL, $product_id, $product_variant, $price, $list_price, $category_id, $quantity, $quantity_total, $height, $width, $length, $gross_weight, $quantity_per_package, $crossdocking, '$created_at', '$updated_at')";
            }
            $insert .= ";";
            ...
            $dir = getcwd() . "/$dir";
            if (is_dir($dir)) {
                file_put_contents("$dir/ms_shipping_pivots_vtex_ult_envio" . date("Ymd_His") . "_$offset-$iteration.sql", $insert);
            }
        }
    }
}
