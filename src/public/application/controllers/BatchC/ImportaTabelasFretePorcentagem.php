...
class ImportaTabelasFretePorcentagem extends BatchBackground_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('model_freights_by_tipo_volume');
    }
	...
	function gravaTipoVolume($sheet) {
		...
		$linhas = $this->lerExcel(FCPATH."/importacao/Tabelas_Transportadoras_Porcentagem.xlsx",$sheet,0);
		...
		foreach($linhas as $linha) {
			...
			while(array_key_exists($i,$linha)) {
				...
				if  ($linha[$i] != 0) {
					...
					$this->model_freights_by_tipo_volume->update($data);
				}
	...
	function alteraCapitais($sheet) {
		...
		$linhas = $this->lerExcel(FCPATH."/importacao/Tabelas_Transportadoras_Porcentagem.xlsx",$sheet,1);
		foreach ($linhas as $linha) {
			$capital = false;
