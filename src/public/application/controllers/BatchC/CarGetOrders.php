...
*/   
class CarGetOrders extends BatchBackground_Controller {
    ...
    function getorders()
    {
		...
		$start_update_date = date('Y-m-d\TH:i:s', strtotime('-6 hours', strtotime('now')));
		$end_update_date= date("Y-m-d\TH:i:s",time() );
		
		$offset = 0;
		while (true) {
			$url = 'https://'.$this->getSite().'/api/orders?max=10&offset='.$offset.'&start_update_date='.$start_update_date.'&end_update_date='.$end_update_date.'&order_state_codes='.$order_state_codes;
			echo 'url='.$url."\n";
			$offset += 10;
