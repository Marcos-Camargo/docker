...
class GrupoSomaProduct extends Main
{
	var $int_to;
	var $auth_data;
	var $score_min = 100;
    ...
    // php index.php BatchC/SellerCenter/GrupoSoma/GrupoSomaProduct run null Farm
	function run($id=null,$params=null)
	{
		...
		if (!is_null($params)) {
            ...
            $catalogs_associated = array(array('int_to' => $params, 'name' => $params, 'is_assciated' => '0'));
            $catalogs_associated = array_merge($catalogs_associated, $this->model_products_catalog_associated->getProductAssociateByIntTo($params));

            foreach ($catalogs_associated as $catalog_check) {
                $this->int_to = $catalog_check['int_to'];

                $catalog = $this->model_catalogs->getCatalogByName($catalog_check['name']);
                if (!$catalog) {
                    echo "Este catálogo ".$this->int_to." não existe\n";
                    if ($catalog_check['is_assciated'] == 0) {
                        echo "Catálogo ".$this->int_to." é o principal e não existe\n";
                        break;
                    }
                    continue;
                }
                ...
                if ($catalog_check['is_assciated'] == 0) {
                    $this->catalog_id_original = $catalog['id'];
                }
                ...
                if (!property_exists($this->auth_data, 'accountName')) {
                    echo "Integração para $this->int_to mal configurada.\n";
                    continue;
                }

                echo "Integração: $this->int_to | ID do Catálogo: $this->catalog_id | Nome do Catálogo: $catalog_check[name]\n";

                $this->accountName = $this->auth_data->accountName;
                $this->getScore();
                $this->notifyPriceAndStockChange($catalog_check['is_assciated'] == 1);
                $this->skuSuggestionInsertion($catalog_check['is_assciated'] == 1);
            }
		}
		else {
    ...
    public function notifyPriceAndStockChange(bool $is_assciated)
    {
        $log_name =$this->router->fetch_class().'/'.__FUNCTION__;
		...
		while ($exists) {
            ...
            if ($is_assciated) {
                $products = $this->model_products_catalog_associated->getProductsChangedToIntegrate($this->int_to, $offsetX, $limit);
            } else {
			...
			foreach ($products as $key => $product) {
                ...
                $blocked = $this->checkBlackList($product, $this->int_to) == 0;
                if ($blocked) {
                    $msg = "Produto $product[id] Produto Bloqueado para $this->int_to";
                    echo "$msg\n";
                    $this->errorTransformation($product['id'], $product['pi_skumkt'], $msg);
                    $product['qty'] = 0;
                }
				...
				if ($this->responseCode == 500) { /// Vtex da o erro 500 e tem q tentar de novo.... 
					$erro = 'notifyPriceAndStockChange: Erro httpcode: '.$this->responseCode.' ao chamar '.$endPoint;
					echo $erro." dormindo 5 segundos \n";
				...
				if (!is_null($is_assciated ? $product['catalog_product_id'] : $product['product_catalog_id'])) {
					$prd_catalog = $this->model_products_catalog->getProductProductData($is_assciated ? $product['catalog_product_id'] : $product['product_catalog_id']);
    ...
    public function skuSuggestionInsertion(bool $is_assciated)
    {
        $log_name =$this->router->fetch_class().'/'.__FUNCTION__;
		...
		while ($exists) {
            ...
            if ($is_assciated) {
                $products = $this->model_products_catalog_associated->getProductsChangedToIntegrateNew($this->int_to, $this->catalog_id_original, $offset, $limit);
            } else {
			...
			foreach ($products as $key => $product) {
				...
				if ($this->responseCode == 500) { /// Vtex da o erro 500 e tem q tentar de novo.... 
					$erro = 'skuSuggestionInsertion: Erro httpcode: '.$this->responseCode.' ao chamar '.$endPoint;
					echo $erro." dormindo 60 segundos \n";
					var_dump($this->result);
					$this->log_data('batch',$log_name, $erro ,"E");
					sleep(60);
					$skuExist = $this->processNew($this->auth_data, $endPoint, 'POST', $bodyParams, $product['id'], $this->int_to, 'Notificação de mudança'); 				
				...
				$images = $product['is_on_bucket'] ? $this->getProductImagesBucket($product['image'], $pathImage) : $this->getProductImages($product['image'], $pathImage);
				if ($images === null) {
				...
				$categoryId = json_decode($product['category_id']);
           		$category   = $this->model_category->getCategoryData($categoryId[0] ?? $categoryId);
           		// pego a categoria do marketplace
				$result= $this->model_categorias_marketplaces->getCategoryMktplace($this->int_to,$categoryId);
				if (!$result) {
                    $categoryIdEncode = json_encode($categoryId);
					$msg= 'Categoria '.$categoryIdEncode.' não vinculada ao marketplace '.$this->int_to;
					$this->errorTransformation($product['id'],$skumkt,$msg);
					continue;
				}
    ...
    private function skuToMatch($product, $sellerId, $idCat, $skumkt)
    {
        ...
        $version = $this->getVersionGS($sellerId, $skumkt, $product['id']);
		...
		$pid = isset($result->ProductId) ? $result->ProductId : $result->productId;
		$sid = isset($result->SkuId) ? $result->SkuId : $result->skuId;

        return (array('success' => true, 'ProductId' => $pid, 'SkuId' => $sid)) ;
    }

    private function getVersionGS($sellerId, $sku, $product_id)
    {
		$log_name =$this->router->fetch_class().'/'.__FUNCTION__;
    ...
    {
		$images = is_dir(FCPATH.'assets/images/'.$path.'/'.$folder) ? scandir(FCPATH.'assets/images/'.$path.'/'.$folder) : false;
	...
	private function getProductImagesBucket($folder, $path)
	{
		$images = $this->bucket->isDirectory('assets/images/' . $path . '/' . $folder) ?
			$this->bucket->getFinalObject('assets/images/' . $path . '/' . $folder)
			: false;

		if (!$images) {
			return null;
		}
		if (count($images['contents']) == 0) {
			return false;
		}
		$numft = 0;
		$imagesData = [];
