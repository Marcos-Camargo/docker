...
class BrandsDownload extends Main
{
  ...
  public function __construct()
	...
	// php index.php BatchC/SellerCenter/Vtex/BrandsDownload run null CasaeVideo
		...
		$this->log_data('batch', $log_name, 'start ' . trim($id . " " . $params), "I");	
		 ...
		 if(is_null($params)  || ($params == 'null')){
			...
			if($integration){
                ...
                $catalog = $this->model_catalogs->getByIntTo($integration['int_to']);
                $inactive_products_with_inactive_brands = false;
                $catalog_id = null;
                if ($catalog) {
                    $inactive_products_with_inactive_brands = $catalog['inactive_products_with_inactive_brands'];
                    $catalog_id = $catalog['id'];
                }
				// coloco o brand como ativo se tiver pelo menos 1 ativo ou inativo se não tiver nenhum ativo
				$this->model_brands_marketplaces->setBrandsActiveOrInactive($inactive_products_with_inactive_brands, $catalog_id);
            }
			else {
	...
	function updateIntTo($integration, $brand)
	{
		...
		$this->processNew($this->auth_data, $endPoint, 'PUT', $data);
	...
	function uploadIntTo($integration, $brand)
	{
		...
		$data = json_encode($data);
		$this->auth_data = json_decode($integration['auth_data']);
		$this->processNew($this->auth_data, $endPoint, 'POST', $data);
		if ($this->responseCode != 200) {
			$erro = "httpcode = " . $this->responseCode . " ao chamar endpoint " . $endPoint . " para pegar brands";
			echo $erro . "\n";
			$this->log_data('batch', $log_name, $erro, "E");
	...
	function syncIntTo($integrationData)
	{
		...
		for ($page=1; $page <= $pages; $page++) {
			$endPoint  = 'api/catalog_system/pvt/brand/pagedlist?page='.$page;
			$this->auth_data = json_decode($integrationData['auth_data']);
			$this->processNew($this->auth_data, $endPoint);
			...
			foreach ($brands['items'] as $key => $brand) {
				...
				$brand_save = $brand['name'];
				$brand['name'] = str_replace("'", "&#039", $brand['name']); 
				
				if ($brand_save != $brand['name']) {  // estava com ' então vou acertar 
					$localbrand = $this->model_brands->getBrandDatabyName($brand_save);
					if (!is_null($localbrand)) {
						echo "Acertando ".$localbrand['id']." de ".$brand_save." para ".$brand['name']."\n";  
						$this->model_brands->update(array('name' => $brand['name']), $localbrand['id']);
						$this->model_brands_marketplaces->update(array('name' => $brand['name']), $integrationData['int_to'], $localbrand['id']);
					}
				}
		...
		$this->createXls($this->brands_mapping);
	}
	...
	private function createXls($mapping) {
