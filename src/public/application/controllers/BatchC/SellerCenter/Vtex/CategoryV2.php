...
class CategoryV2 extends Main
{
    ...
    // php index.php BatchC/SellerCenter/Vtex/CategoryV2 run null CasaeVideo
    ...
    function syncIntTo($id_integration, $int_to, $auth_data) {
		...
		if (($this->responseCode == 429) || ($this->responseCode == 504) || ($this->responseCode == 502)) {
			echo " VTEx respondeu ".$this->responseCode.". Aguardando 60 segundos\n";
            sleep(60);
			$categories_json = $this->getCategories($int_to, $auth_data, $endPoint);
		}
        ...
        If ($category_market) { // Categoria existe, vamos ver se mudou o nome             
            ...
            if ($categories['name'] != $breadcrumb) {
                ...
                if (!count($all_categories)) {
                    echo "O nome mudou na Vtex de '".$breadcrumb."' para '".$categories['name']."'. Acertando a categoria\n";
                    ...
                    echo "O nome mudou na Vtex de '".$breadcrumb."' para '".$categories['name']."' Como tem mais de uma categoria associada, o nome da categoria não mudará.\n";                                            
                }                                
            }
    ...
    private function findAndSaveSpecifications($int_to, $auth_data, $id_integration, $category_id, $category_mkt_id) {
       ...
       // $endPoint = 'api/catalog_system/pub/specification/field/listByCategoryId/'. $category_mkt_id;
		...
		if (($this->responseCode == 429) || ($this->responseCode == 504) || ($this->responseCode == 500) || ($this->responseCode == 503)|| ($this->responseCode == 502)) {
            echo "Vtex com erro ".$this->responseCode."\n";
			sleep(60);
			$specifications_json = $this->getSpecifications($int_to, $auth_data, $endPoint);
		}
		if ($this->responseCode !== 200) {
		...
		$this->model_atributos_categorias_marketplaces->removeByIntToCategory($int_to, $category_mkt_id);
        ...
        foreach ($specifications as $specification) {
            ...
            $specification_data = array(
                ...
                'valor'             => json_encode($values, JSON_UNESCAPED_UNICODE),
                'atributo_variacao' => 0
            );
    ...
    private function getSpecifications($int_to, $auth_data, $end_point) {
        ...
        if (($this->responseCode == 429) || ($this->responseCode == 504)) {
            echo "Timeout ".$this->responseCode."\n";
			sleep(60);
			return $this->getSpecifications($int_to, $auth_data, $end_point);
		}
    ...
    private function checkMissingCategories($int_to) 
    {
        ...
        echo "Verificando categorias que não existem mais na Vtex \n";
        if (!$inactivate_category) {
