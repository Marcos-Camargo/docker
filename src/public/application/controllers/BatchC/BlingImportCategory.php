...
*/   
class BlingImportCategory extends BatchBackground_Controller {
	...
	function getBlingKeys() {
		$apikeys = Array();
		//$sql = "select * from stores_mkts_linked where id_integration = 13 AND apelido!='B2W' AND apelido!='CAR'";
    ...
    */
    function syncCategories($apikey)
    {
		...
		*/		    
		...
		//$sql = "select * from categories ORDER BY id ";       // CHANGED IN THE LAST HOUR
		$query = $this->db->query($sql);
		$data = $query->result_array();
		foreach ($data as $key => $row) 
	    {
