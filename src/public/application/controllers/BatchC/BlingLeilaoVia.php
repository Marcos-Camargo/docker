...
*/   
 class BlingLeilaoVia extends BatchBackground_Controller {
		...
		//	if ($qty_atual<$parms[$val['int_to']]) { // Não tem mais estoque mínimo por marketplace por enquanto
			...
			//if (($qty_atual==0) || ((int) $prd['qty'] < 5)) {  
	        	...
	        	$data = array(
	        		...
	        		'prd_id' => $val['prd_id'],
	        		'EAN' => $ean,
	        		'price' => $this->model_promotions->getPriceProduct($val['prd_id'],$prd['price']),
	        		'qty' => $prd['qty'],
	        		'sku' => $prd['sku'],
    ...
    */
    ...
    function syncProducts()
    {
		...
		$sql = "select * from prd_to_integration where status_int=10 AND status=1 AND int_type = 13 ".$not_int_to." ORDER BY int_to";
		$query = $this->db->query($sql);
...
//			echo 'row[int_to]='.$row['int_to'];
			...
			$row_int_pro['prd_id'] = $prd['id'];
			$row_int_pro['EAN'] = $ean;
			$row_int_pro['qty_atual'] = 0;
			$row_int_pro['qty'] = 0;
			$row_int_pro['price'] = $prd['price'];
			$row_int_pro['sku'] = $prd['sku'];
			$row_int_pro['company_id'] = $prd['company_id'];
			$row_int_pro['reputacao'] = 100 ; 
			$row_int_pro['NVL'] = 2139;
			$row_int_pro['int_to'] = 'VIA';
			...
			*/
			...
			$sql = "UPDATE prd_to_integration SET status_int=10 , date_last_int = ? WHERE id = ".$row['id'];
        	...
        	$data = array(
        		...
        		'NVL' => $row_int_pro['NVL'],
        		'mkt_store_id' => 203402233,  //$mktVS[$row['int_id']],  
