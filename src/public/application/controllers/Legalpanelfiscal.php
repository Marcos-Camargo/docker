...
class Legalpanelfiscal extends Admin_Controller
{
    public function __construct()
    {
        ...
        $this->data['page_title'] = $this->lang->line('application_legal_panel_fiscal');
    ...
    {
        if(!in_array('viewLegalPanelFiscal', $this->permission)) {
            redirect('dashboard', 'refresh');
        }
        $this->render_template('legalpanelfiscal/index', $this->data);
    }
    ...
    public function fetchLegalPanelData()
    {
		...
		$data = $this->model_legal_panel_fiscal->getAll($status);
		...
		foreach ($data as $key => $value) {
			...
			if(in_array('updateLegalPanelFiscal', $this->permission)) {
				$buttons .= ' <a href="'.base_url('legalpanelfiscal/edit/'.$value['id']).'" class="btn btn-sm btn-default"><i class="fa fa-edit"></i></a>';
			}
			if(in_array('viewLegalPanelFiscal', $this->permission)) {
			$buttons .= ' <a href="'.base_url('legalpanelfiscal/read/'.$value['id']).'" class="btn btn-sm btn-default"><i class="fa fa-eye"></i></a>';
			}
			if(in_array('deleteLegalPanelFiscal', $this->permission)) { 
    ...
    public function create()
	{
		...
		if(!in_array('createLegalPanelFiscal', $this->permission)) {
            redirect('dashboard', 'refresh');
        }
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$response = $this->model_legal_panel_fiscal->create($data);
			redirect('legalpanelfiscal/', 'refresh');
			...
			$this->render_template('legalpanelfiscal/create', $this->data);
	...
	public function edit($id = null)
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$response = $this->model_legal_panel_fiscal->update($data, $id);
			redirect('legalpanelfiscal/', 'refresh');

		}else{
			
			$this->data['read_only'] = '';
			$legal_panel = $this->model_legal_panel_fiscal->getDataById($id);
			$this->data['legal_panel'] = $legal_panel;
	...
	public function fileUpload()
	{
	    ...
	    if (!empty($_FILES)) {
			...
			$caminhoMapeado = $serverpath . 'assets/docs/legalpanelfiscal/';
    ...
    {
        if (!in_array('createLegalPanelFiscal', $this->permission) && !in_array('updateLegalPanelFiscal', $this->permission)) {
            redirect('dashboard', 'refresh');
        }
        ...
        $ln2 = array();
        $fotos = array();
		$targetPath = str_replace('\\','/',FCPATH . 'assets/docs/legalpanelfiscal/' . $token);
		...
		if(is_dir($targetPath)){
			...
			foreach ($fotos as $foto) {
				if (($foto != ".") && ($foto != "..") && ($foto != "")) {
					...
					array_push($ln1, [base_url("assets/docs/legalpanelfiscal/".$token."/" . $foto )]);
					array_push($ln2, (object) array( 'key' => $token."/" . $foto, 
						'downloadUrl' => base_url("assets/docs/legalpanelfiscal/".$token."/" . $foto),
						'type' => $extensao, 
						'caption' => $foto )
					);
				}
