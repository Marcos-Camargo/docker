<?php
/*
 Controller de Produtos Devolvidos
 */
...
class ProductsReturn extends Admin_Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->data['page_title'] = $this->lang->line('application_return_order');
        $this->data['page_now'] = $this->lang->line('application_return_order');
        $this->data['pageinfo'] = $this->lang->line('application_return_order');
	...
	*/
    public function fetchOrdersData($postdata = null)
    {
            ...
            if ($busca['value']) {
                ...
                if (trim($postdata['entrega'])) {
                    if ($postdata['entrega'] == 'CORREIOS') {
                        $this->data['ordersfilter'] .= " AND f.ship_company = 'CORREIOS' ";
                    } else {
                        $this->data['ordersfilter'] .= " AND f.ship_company <> 'CORREIOS' ";
                    }
                }
        ...
        foreach ($data as $key => $value) {
            ...
            elseif($value['paid_status'] == 59) { // Cliente deve retirar o objeto no local
            ...
            if ($postdata['internal'] == 'false') {
                ...
                if ($show_marketplace_order_id == 1) {
                    $result[$key] = array(
	                    ...
	                    ($value['ship_company'] == '' ? ($value['ship_company_preview'] == 'CORREIOS' ? 'CORREIOS' : 'TRANSPORTADORA') : ($value['ship_company'] == 'CORREIOS' ? 'CORREIOS' : 'TRANSPORTADORA')),
	                    $value['store'],
                        $paid_status,
	...
	*/
    ...
    {
        if (!in_array('createOrder', $this->permission) || ENVIRONMENT != 'development')
            redirect('dashboard', 'refresh');
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (!$client_id) {
                $this->db->trans_rollback();
                $this->session->set_flashdata('errors', 'Erro para inserir o cliente!');
            ...
            for ($iten = 0; $iten < count($this->postClean('product_id')); $iten++) {
                ...
                if (!$dataProduct) {
                    $this->db->trans_rollback();
                    $this->session->set_flashdata('errors', 'Erro para encontrar o produto!');
                    redirect('orders/create', 'refresh');
                }
            ...
            if (!$order_id) {
                $this->db->trans_rollback();
                $this->session->set_flashdata('errors', 'Erro para criar o pedido!');
                redirect('orders/create', 'refresh');
            }
            ...
            foreach ($orderItem as $key => $iten) {
                ...
                if (!$order_iten) {
                    $this->db->trans_rollback();
                    $this->session->set_flashdata('errors', 'Erro para criar o item do pedido!');
                    redirect('orders/create', 'refresh');
                }
            }
    ...
    public function update($id)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            foreach ($orders_item as $k => $v) {
                ...
                //if ($v['image']) {
                //    $files = scandir(substr($_SERVER['SCRIPT_FILENAME'], 0, strpos($_SERVER['SCRIPT_FILENAME'], 'index.php')).'assets/images/product_image/'.$v['image']);
                //}
                // $result['order_item'][$k]['picture'] = $files[2] ?? null;
                ...
                if ($v['variant'] != '') {
                    ...
                    if (strpos($product_data['has_variants'], ";") > 0) {
                        ...
                        foreach ($variants_names as $variants_name) {
                            if (isset($variants_value[$i_var]) && $variants_value[$i_var] != 0) {
            ...
            foreach ($cancelPenaltiesTo as $cancelPenaltyTo) {
                if (!in_array('admDashboard', $this->permission) && $cancelPenaltyTo['value'] != '1-Seller') continue;

                array_push($arrPenalties, $cancelPenaltyTo['value']);
            }
            ...
            if ($orders_data['paid_status'] == 58) {
                $this->data['occurrence'] = $result['frete_ocorrencias'][count($result['frete_ocorrencias']) - 1];
            }
    ...
    public function updatenfe()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $data = array(
                ...
                'chave' => $this->postClean('chave_nfe'),
            ...
            $chave_acesso = $this->verificaChave($data);
            if (!$chave_acesso['ok']) {
                $response['success'] = false;
                $response['messages']['chave_nfe'] = "<p class='text-danger'>" . $chave_acesso['mensagem'] . "</p>";
                $semerro = false;
            }
        ...
        $this->log_data(__CLASS__, __FUNCTION__, 'form_data='.json_encode($this->postClean()), "I");
        ob_clean();
        echo json_encode($response);
    }
    ...
    public function fetchOrdersDataSemFrete($postdata = null)
    {
        ...
        if (isset($this->data['ordersfilter'])) {
            $filtered = count($this->model_orders->getOrdersSemFreteByLike($whereLike));
        } else {
            $filtered = 0;
        }
        ...
        $data = $this->model_orders->getOrdersSemFreteByLike($whereLike, [], $order_by, $order_dir);
    ...
    public function novofrete($id)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->data['ship_companies'] = $this->model_shipping_company->getShippingCompanyActiveByStoreAndSellerCenter($orders_data['store_id']);
    ...
    public function freteEntregueSelect()
    {
        ...
        redirect('orders/freteEntregue', 'refresh');
        //$this->render_template('orders/freteentregue', $this->data);
    }
    ...
    public function freteEntregueData()
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $linkid = '<a href="' . base_url() . 'orders/view/' . $value['id'] . '" target="_blank">' . $value['id'] . '</a>';
            $buttons = '<a target="__blank" href="' . base_url('orders/view/' . $value['id']) . '" class="btn btn-default"><i class="fa fa-eye"></i></a>';
    ...
    public function cancelarPedido()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $cancelPenaltiesTo = $this->model_attributes->getAttributeValuesByName('cancel_penalty_to');
            foreach ($cancelPenaltiesTo as $cancelPenaltyTo) {
                if ($this->postClean('penalty_to') == $cancelPenaltyTo['value']) {
                    $penaltyExist = true;
                    break;
                }
            }
            if (!$penaltyExist || (!in_array('admDashboard', $this->permission) && $this->postClean('penalty_to') != '1-Seller')) {
                $response['success'] = false;
            ...
            $data = array(
                ...
                'status' => '1',
                'penalty_to' => $this->postClean('penalty_to', TRUE),
                'user_id' => $this->session->userdata('id')
            );
    ...
    public function cancelaMktSelect()
    {
        ...
        redirect('orders/cancelaMkt', 'refresh');
        //$this->render_template('orders/freteentregue', $this->data);
    }
    ...
    public function envioMktSelect()
    {
        ...
        redirect('orders/envioMkt', 'refresh');
    }
    ...
    public function invoiceMktSelect()
    {
        ...
        redirect('orders/enviadoNfeMkt', 'refresh');
    }
    ...
    public function manage_tags()
    {
        ...
        if ($this->data['usercomp'] == 1) { // admin
            ...
            $stores = $this->model_stores->getCompanyStores($this->data['usercomp']);
            foreach ($stores as $store)
    ...
    public function manage_tags_post()
    {
        ...
        foreach ($orders as $order) {
            ...
            if (!$store_verify) $store_verify = $orderDb['store_id'];
            else {
            ...
            foreach ($rastreios as $rastreio) {
                array_push($arrObjetosPlp, array('objeto' => $rastreio['codigo_rastreio']));
        ...
        if ($plpAutomatic || in_array($store_verify, $storesException)) {
            ...
            foreach ($arrObjetosPlp as $objeto) {
                ...
                if (!array_key_exists($_plp_temp, $arrPlps)) $arrPlps[$_plp_temp] = array();

                array_push($arrPlps[$_plp_temp], array(
        ...
        if ($plpAutomatic || in_array($store_verify, $storesException)) {
            foreach ($plps as $plp) {
                ...
                if ($plp->erro) {
                    ...
                    array_push($objetosError, $plp->objeto);

                    //cancelar etiqueta dos pedidos para gerar novamente
                    array_push($ordersError, $orders_plp['order_id']);
                    array_push($messagesError, $plp->erro);
                    continue;
                }

                if (!array_key_exists($plp->plp, $arrPlps)) $arrPlps[$plp->plp] = array();

                array_push($arrPlps[$plp->plp], array(
            ...
            $this->generatePdfEtiquetas($arrPlps, $dataStore);
        }
    ...
    public function fetchEtiquetas()
    {
        $dataEtiqueta = $this->model_orders->getEtiquetasGeneratePLP();
        $etiquetasTemp = array();
        $etiquetas = array();
    ...
    public function getTagsTransmit()
    {
        $dataPlp = $this->model_freights->getDataPlpTransmit();
        $plps = array();

        foreach ($dataPlp as $item) {
            if(!key_exists($item['number_plp'], $plps)) {
                $button = $item['status'] == 0 ? '<button type="button" class="btn btn-success btn-sm-tag viewTags" number-plp="' . $item['number_plp'] . '"><i class="fa fa-tag"></i> ' . $this->lang->line('application_labels') . '</button>' :
                    '<a href="' . $item['link_plp'] . '" class="btn btn-success btn-sm-tag" download><i class="fas fa-file-powerpoint"></i> PLP </a>
                ...
                $plps[$item['number_plp']] = array(
                    $item['number_plp'] == 0 || $item['status'] == 0 ? $this->lang->line('application_in_progress') : $item['number_plp'],
                ...
                $plps[$item['number_plp']][1] .= "<a class='btn btn-primary btn-sm-tag' href='" . base_url('orders/update/' . $item['order_id']) . "' target='_blank'>{$item['order_id']}</a>";
        }

        ob_clean();
        echo json_encode($plps);
    }
    ...
    public function generatePlp($plp)
    {
        ...
        if ($dataCorreios) {
            ...
            foreach ($servicos as $servico) {
                $htmlServicos .= "
            <tr>
                <td style='width: 20%; padding-left: 10px; font-size: 13px;padding-top: 15px'>{$dataCorreios->cartao}</td>
                        ...
                        <tbody>
                            <tr>
                                <td style='width: 75%; padding-top: 20px'>
                                    <span>Contrato: {$dataCorreios->contrato}</span><br>
    ...
    public function generatePdfEtiquetas($arrObjetosAll, $dataStore)
    {
        $logistic = $this->calculofrete->getLogisticStore(array(
            'freight_seller' 		=> $dataStore['freight_seller'],
            'freight_seller_type' 	=> $dataStore['freight_seller_type'],
        ...
        foreach ($arrObjetosAll as $plp => $itens) {
            ...
            if (!array_key_exists($plp, $arrObjetos)) $arrObjetos[$plp] = array();

            foreach ($itens as $iten)
                array_push($arrObjetos[$plp], $iten['code_tracking']);
        }
    ...
    public function createNewExchange($order_id, $order_itens)
    {
        ...
        foreach ($order_itens as $key => $order_item) {
            $orders_its = $this->model_orders->getItensByOrderId($order_id);
    ...
    public function getStrZplByTrackinAndOrder($trackins, $order_id)
    {
        ...
        $dataNfe    = $this->model_nfes->getNfesDataByOrderId($order_id);
...
^CF0,33 ^FO60,560^FD{$this->tirarAcentos($dataOrder['customer_address'])}, {$this->tirarAcentos($dataOrder['customer_address_num'])} Bairro: {$this->tirarAcentos($dataOrder['customer_address_neigh'])}^FS
^CF0,33 ^FO60,610^FDCEP: {$dataOrder['customer_address_zip']} - {$this->tirarAcentos($dataOrder['customer_address_city'])} - {$this->tirarAcentos($dataOrder['customer_address_uf'])}^FS
...
^CF0,35 ^A0N^FO280,820^FD{$this->tirarAcentos($dataStore['raz_social'])}
^CF0,33 ^FO60,860^FD{$this->tirarAcentos($dataStore['address'])}, {$this->tirarAcentos($dataStore['addr_num'])}^FS
^CF0,33 ^FO60,900^FDBairro: {$this->tirarAcentos($dataStore['addr_neigh'])}^FS
^CF0,33 ^FO60,940^FDCEP: {$dataStore['zipcode']} - {$this->tirarAcentos($dataStore['addr_city'])} - {$this->tirarAcentos($dataStore['addr_uf'])}^FS
...
^XZ";
    ...
    public function sendFreightHire()
    {
        ...
        if (!$delFreight || !$updateStatus || !$createComment) {
            $this->db->trans_rollback();
            $this->session->set_flashdata('errors', "Ocorreu um problema para atualizar o pedido. Tente novamente!");
            redirect('orders/update/' . $order_id, 'refresh');
        }
    ...
    public function createTagTransp($orders)
    {
        ...
        foreach ($orders as $order) {
            ...
            foreach ($freights as $freight) {
                ...
                /*if ($freight['ship_company'] != 'CARRIERS LOGISTICA E TRANSPORTE LTDA' && $freight['ship_company'] != 'CARRIERS') {
                    $html = "<div style='width: 100%; text-align: center;margin-top: 10%'><h2 style='font-family: 'Microsoft YaHei','Source Sans Pro', sans-serif;'>Não encontramos informações para a geração da etiqueta.</h4></div>";
                    $dompdf->loadHtml($html);
                    $dompdf->render();
                    $dompdf->stream(
                        'Etiquetas' . implode('-', $orders) . '.pdf',
                        array(
                            "Attachment" => false
                        )
                    );
    ...
    public function requestGenerateTagShippingCompany($orders = '')
    {
        $orders = explode('-', $orders);
    ...
    public function fetchEtiquetasTransp()
    {
        $storesFilter       = $this->postClean('stores') ?? array();
        $ship_companyFilter = $this->postClean('ship_company') ?? array();
        $dataEtiqueta       = $this->model_orders->getEtiquetasCarrier($storesFilter, $ship_companyFilter);
        $etiquetasTemp      = array();
        $etiquetas          = array();
        $logistic           = array();
        ...
        foreach ($dataEtiqueta as $item) {
            ...
            if (!array_key_exists($item['store_id'], $logistic)) {
                ...
                $logistic[$item['store_id']]['isLogisticTypeWithAutoFreightAcceptedGeneration'] = $this->logisticAutoApproval
                    ->isLogisticTypeWithAutoFreightAcceptedGeneration(
                        $logistic[$item['store_id']]['type']
                    );
            }

            if (!Model_orders::isFreightAcceptedGeneration($item) &&
                !$logistic[$item['store_id']]['isLogisticTypeWithAutoFreightAcceptedGeneration']
            ) {
                continue;
            }
            ...
            if (
                (
                    $logistic[$item['store_id']]['type'] !== 'sgpweb' ||
                    (
                        $logistic[$item['store_id']]['type'] !== 'sellercenter' &&
                        $logistic[$item['store_id']]['sellercenter']
                    )
                )
            ) { // É por transportadora ou getaway logístico e ainda não foi autorizado a emissão
                if (!array_key_exists($item['order_id'], $etiquetasTemp)) {
                    $etiquetasTemp[$item['order_id']] = array(
                ...
                } else $etiquetasTemp[$item['order_id']][5] = "{$etiquetasTemp[$item['order_id']][5]}<br>{$item['codigo_rastreio']}";
            }
        }

        foreach ($etiquetasTemp as $etiqueta) array_push($etiquetas, $etiqueta);

        ob_clean();
        echo json_encode($etiquetas);
    }

    public function fetchEtiquetasWithoutTransp()
    {
        $storesFilter = $this->postClean('stores') ?? array();

        $dataEtiqueta   = $this->model_orders->getEtiquetasWithoutCarrier($storesFilter);
        $etiquetas      = array();
        $logistic       = array();
...
//            dd($this->postClean(), $this->postClean('sendOrderToIntegration'), $this->postClean('updateStockNewsProducts'));
    ...
    public function updateOrderCancelRequestCancel()
    {
        ...
        $user_id     = $this->session->userdata('id');
        $reason      = $this->postClean('reason_cancel_request');
        $lastRequest = $this->model_requests_cancel_order->getLastReasonByOrder($order);
    ...
    // Retorna o conteúdo do arquivo "orders/return.php", que contém a interface de acompanhamento de produtos devolvidos.
    public function return()
    {
        ...
        $this->data['show_marketplace_order_id'] = $this->model_settings->getStatusbyName('show_marketplace_order_id');
        $this->render_template('orders/return', $this->data);
        // $this->render_template('products/return', $this->data);
    }

    public function fetchReturnOrders($postdata = null)
    {
        // Se o usuário não tem permissão de acesso, ele é redirecionado para o dashboard.
        if (!in_array('viewReturnOrder', $this->permission)) {
            redirect('dashboard', 'refresh');
        }

        // Recupera as informações enviadas através do método "post".
        $postdata = $this->postClean();
        ...
        // Variável usada para controlar quais filtros foram usados e com quais valores.
        // Esta variável será usada na consulta ao banco de dados.
        $whereClauses = [];

        // Função auxiliar para adicionar cláusulas WHERE.
        $addWhereClause = function($condition) use (&$whereClauses) {
            if (!empty($condition)) {
                $whereClauses[] = $condition;
            }
        };

        // Se foi usado o filtro "Pedido Conecta Lá".
        $pedido_conecta = $postdata['pedidoconecta'];
        $addWhereClause(!empty($pedido_conecta) ? "product_return.order_id LIKE '%" . $this->db->escape_like_str($pedido_conecta) . "%'" : "");

        // Se foi usado o filtro "Pedido Marketplace".
        $pedido_mkt = $postdata['pedidomkt'];
        $addWhereClause(!empty($pedido_mkt) ? "orders.numero_marketplace LIKE '%" . $this->db->escape_like_str($pedido_mkt) . "%'" : "");

        // Se foi usado o filtro "Lojas".
        $lojas = $postdata['stores'];
        if (!empty($lojas)) {
            $lista_lojas = implode(', ', array_map([$this->db, 'escape'], $lojas));
            $addWhereClause("orders.store_id IN ($lista_lojas)");
        }

        // Se foi usado o filtro "Transportadora".
        $trans = $postdata['shippingco'];
        if (!empty($trans)) {
            $transportadora = strtolower($trans[0]);
            $addWhereClause("lower(product_return.logistic_operator_type) = lower('" . $this->db->escape_str($transportadora) . "')");
        }

        // Se foi usado o filtro "Status".
        $sta = $postdata['status'];
        if (!empty($sta)) {
            $status = strtolower($sta[0]);
            if (strpos($status, 'contratar') !== false) {
                $status = 'a_contratar';
            }
            $addWhereClause("lower(product_return.status) LIKE lower('%" . $this->db->escape_like_str($status) . "%')");
        }

        // Se foi usado o filtro "Data de envio para contratação".
        $data_envio_contrata = $postdata['hiredatereturn'];
        if (!empty($data_envio_contrata)) {
            $data_envio_contrata = date("Y-m-d", strtotime($data_envio_contrata));
            $addWhereClause("product_return.devolution_request_date LIKE '%" . $this->db->escape_like_str($data_envio_contrata) . "%'");
        }

        // Se foi usado o filtro "Data contratada".
        $data_contrata = $postdata['hiredate'];
        if (!empty($data_contrata)) {
            $data_contrata = date("Y-m-d", strtotime($data_contrata));
            $addWhereClause("product_return.devolution_contract_date LIKE '%" . $this->db->escape_like_str($data_contrata) . "%'");
        }

        // Se foi usado o filtro "Data de envio".
        $data_envio = $postdata['datereturn'];
        if (!empty($data_envio)) {
            $data_envio = date("Y-m-d", strtotime($data_envio));
            $addWhereClause("product_return.devolution_date LIKE '%" . $this->db->escape_like_str($data_envio) . "%'");
        }

        // Se foi usado o filtro "Pesquisar".
        $search = $postdata['search']['value'];
        if (!empty($search)) {
            $search = strtolower($search);
            $searchClauses = [
                "lower(orders.numero_marketplace) LIKE '%" . $this->db->escape_like_str($search) . "%'",
                "lower(product_return.order_id) LIKE '%" . $this->db->escape_like_str($search) . "%'",
                "lower(product_return.devolution_invoice_number) LIKE '%" . $this->db->escape_like_str($search) . "%'",
                "lower(product_return.sku_marketplace) LIKE '%" . $this->db->escape_like_str($search) . "%'"
            ];
            $addWhereClause("(" . implode(' OR ', $searchClauses) . ")");
        }
        ...
        $query_result = $this->model_product_return->fetchReturnedProducts($where, $limit, $offset);

        // Processamento dos resultados da consulta.
        $result = array();
        foreach ($query_result as $result_key => $result_value) {
            $order_id = $result_value['order_id'];
            $order_path = "<a href='" . base_url("/orders/update/$order_id") . "' target='blank'>$order_id</a>";

            $product_sku = $result_value['sku_marketplace'];
            $product_id = $result_value['product_id'];
            $product_path = !empty($product_id) ? "<a href='" . base_url("/products/update/$product_id") . "' target='blank'>$product_sku</a>" : $product_sku;

            $status_label = "";
            $status_index = 0;
            $already_returned = false;
            if (strtolower($result_value['status']) != 'devolvido') {
                $status_return = strtoupper($result_value['status']);
                $status_hire = '<option value="ACONTRATAR" style="color: #000; background-color: #f9f9f9;">A contratar</option>';
                if (strpos($status_return, 'CONTRATAR') !== false) {
                    $status_hire = '<option value="ACONTRATAR" style="color: #000; background-color: #f9f9f9;" selected>A contratar</option>';
                    $status_label = '<select class="form-control select2" id="buscastatus' . $result_value['id'] . '" onchange="atualizastatus(\'' . $result_value['id'] . '\', 0,\'' . $result_value['order_id'] . '\')" style="color: #fff; background-color: #ffc107;">';
                    $status_index = 0;
                }

                $status_collected = '<option value="COLETADO" style="color: #000; background-color: #f9f9f9;">Coletado</option>';
                if (strpos($status_return, 'COLETADO') !== false) {
                    $status_collected = '<option value="COLETADO" style="color: #000; background-color: #f9f9f9;" selected>Coletado</option>';
                    $status_label = '<select class="form-control select2" id="buscastatus' . $result_value['id'] . '" onchange="atualizastatus(\'' . $result_value['id'] . '\', 1,\'' . $result_value['order_id'] . '\')" style="color: #fff; background-color: #007bff;">';
                    $status_index = 1;
                }

                $status_returned = '<option value="DEVOLVIDO" style="color: #000; background-color: #f9f9f9;">Devolvido</option>';
                if (strpos($status_return, 'DEVOLVIDO') !== false) {
                    $already_returned = true;
                    $status_label = '<span class="label label-success">Devolvido</span>';
                    $status_index = 2;
                }

                $status_cancelled = '<option value="CANCELADO" style="color: #000; background-color: #f9f9f9;">Cancelado</option>';
                if (strpos($status_return, 'CANCELADO') !== false) {
                    $already_returned = true;
                    $status_label = '<span class="label label-danger">Cancelado</span>';
                    $status_index = 3;
                }
                ...
                <input type=\"hidden\" id=\"current_index" . $result_value['id'] . "\" value=\"$status_index\">";
                }
            } else {
                $status_label = '<span class="label label-success">Devolvido</span>';
            }
            ...
            $invoice_path = "";
            if (!empty($result_value['return_invoice_file'])) {
                $invoice_path = $this->model_product_return->getURIPath() . '/assets/files/nfe/' . $result_value['return_invoice_file'];
            }

            $invoice_file = !empty($invoice_path) ? "<a href='$invoice_path' target='blank'>" . $result_value['devolution_invoice_number'] . "</a>" : '';

            $result[$result_key] = array(
                "<a href='".base_url("/ProductsReturn/requestReturnOrders/" . $result_value['id']) . "' target='blank'>" . $result_value['id'] . "</a>",
                $order_path,
                $result_value['numero_marketplace'],
                $invoice_file,
                $result_value['logistic_operator_type'],
                $result_value['return_total_value'],
                $result_value['store_name'],
                $product_path,
                $status_label
            );
        }

        $total_records = $this->model_product_return->getTotalReturnedProducts();
        $filtered_records = $this->model_product_return->getTotalReturnedProducts($where);
    ...
    // Retorna o conteúdo do arquivo "orders/requestReturnOrders", que contém a interface de acompanhamento de produtos devolvidos.
    public function requestReturnOrders()
    {
        ...
        $this->data['return_action'] = 'create';
        $id_parameter = $this->uri->segment($this->uri->total_segments());
        // Se a página tem o índice submetido através do método GET.
        if (is_numeric($id_parameter)) {
            $items = $this->model_product_return->getReturnedOrderInformation($id_parameter);

            if (empty($items)) {
                redirect('ProductsReturn/return', 'refresh');
            }

            $post_item = "";
            foreach ($items as $item) {
                $oplog = '<option value="ESCOLHA">Escolha</option>';
                $oplogistico = $item['logistic_operator_type'];
                // 'TRANSPORTADORA', 'CORREIOS'
                $oplogisticos_list = array('TRANSPORTADORA');
                foreach ($oplogisticos_list as $op) {
                    $op = strtolower($op);
                    if (strtolower($oplogistico) == $op) {
                        $oplog .= '<option value="' . strtoupper($op) . '" selected>' . ucfirst($op) . '</option>';
                    } else {
                        $oplog .= '<option value="' . strtoupper($op) . '">' . ucfirst($op) . '</option>';
                    }
                }
                $this->data['oplogistico'] = $oplog;

                if ($this->ms_freight_tables->use_ms_shipping) {
                    try {
                        $this->ms_freight_tables->setStore($item['store_id']);
                        $shipping_companies = $this->ms_freight_tables->getShippingCompanies();
                    } catch (Exception $exception) {
                        $shipping_companies = array();
                    }
                } else {
                    $shipping_companies = $this->model_shipping_company->getShippingCompaniesActive();
                }
                ...
                foreach ($shipping_companies as $shipping_company) {
                    ...
                    if (strtolower($item['logistic_operator_name']) == strtolower($shipping_company_name)) {
                        $ntransportadora .= '<option value="' . $shipping_company_id . '" selected>' . $shipping_company_name . '</option>';
                    } else {
                        $ntransportadora .= '<option value="' . $shipping_company_id . '">' . $shipping_company_name . '</option>';
                    }
                }
                $this->data['ntransportadora'] = $ntransportadora;

                $this->data['ship_service_preview'] = $item['reverse_logistic_code'];
                $this->data['ship_service_price'] = $item['return_shipping_value'];
                $this->data['return_date'] = $item['devolution_request_date'];
                $this->data['return_nfe_number'] = $item['devolution_invoice_number'];
                $this->data['return_reason'] = $item['motive'];
                $this->data['return_action'] = 'edit';
                ...
                $picture = "null";
                if ($item['principal_image']) {
                    $picture = '"' . $item['principal_image'] . '"';
                }

                $quantityOrder = !empty($item['order_quantity']) ? $item['order_quantity'] : (!empty($item['quantity_in_order']) ? $item['quantity_in_order'] : 0);


                $post_item .= '{"product_id":"' . $item['product_id'] . '",' .
                    '"order_id":"' . $item['order_id'] . '",' . 
                    '"store_id":"' . $item['store_id'] . '",' . 
                    '"product_quantity":' . $item['quantity_requested'] . ',' . 
                    '"sku":"' . $item['sku_marketplace'] . '",' . 
                    '"name":"' . $item['name'] . '",' . 
                    '"price":' . ($item['quantity_requested'] * $item['price']) . ',' . 
                    '"picture":' . $picture . ',' . 
                    '"order_quantity":' . $quantityOrder . '}';

            }
            $post_items = "[$post_item]";

        // Se a página não tem variáveis submetidas através do método POST.
        } else if ($this->postClean()) {
            $post_request = $this->postClean();
            $post_items = $post_request['items'];
        } else {
            redirect('dashboard', 'refresh');
        }

        $complete_order = 0;
        if (isset($post_request['complete_order'])) {
            $complete_order = $post_request['complete_order'];
        }

        $returned_items = json_decode(html_entity_decode($post_items), true);
            ...
            <table class="table table-bordered" id="product_info_table">
                <thead>
                    <tr>
                        ...
                        <th style="width:10%"><div data-toggle="tooltip" data-container="body" data-placement="top" title="" data-original-title="">Valor</div></th>
                    </tr>
                </thead>
        ...
        $counter = 0;
        $product_counter = 0;
        foreach ($returned_items as $current_item) {
            if (isset($current_item['order_id'])) {
                $order_id = $current_item['order_id'];
                $product_counter++;
            }

            if (isset($current_item['store_id'])) {
                $store_id = $current_item['store_id'];
                $product_counter++;
            }

            $quantity = -1;
            if (isset($current_item['product_quantity'])) {
                $quantity = $current_item['product_quantity'];
                $product_counter++;
            }

            $order_quantity = -1;
            if (isset($current_item['order_quantity'])) {
                $order_quantity = $current_item['order_quantity'];
                $product_counter++;
            }

            $product_id = -1;
            if (isset($current_item['product_id'])) {
                $product_id = $current_item['product_id'];
                $product_counter++;
            }

            $price = -1;
            if (isset($current_item['price'])) {
            ...
            if ($product_counter == 6) {
                $products_return[$product_id]['order_id'] = $order_id;
                $products_return[$product_id]['store_id'] = $store_id;
                $products_return[$product_id]['quantity'] = $quantity;
                $products_return[$product_id]['order_quantity'] = $order_quantity;
                $products_return[$product_id]['return_price'] = $return_price;
                $product_counter = 0;
            }

            $picture = base_url('/assets/images/system/sem_foto.png');
            if (isset($current_item['picture'])) {
                $picture = $current_item['picture'];
            }

            $name = "";
            if (isset($current_item['name'])) {
                $name = $current_item['name'];
            }

            $sku = "";
            if (isset($current_item['sku'])) {
                $sku = $current_item['sku'];
            }

            if (($sku != "") && ($name != "") && ($price != -1) && ($quantity != -1)) {
                $items_list .= '
                    <tr id="row_' . $counter . '">
                        <td class="link-order-item">
                            <a href="' . base_url() . 'products/update/' . $product_id . '" target="_blank">
                                <span>' . $sku . '</span>
                                <i class="fa fa-eye" aria-hidden="true"></i>
                            </a>
                        </td>
                        <td><img style="width: 60px; height: 60px;" class="img-circle" src="' . $picture . '"></td>
                        <td><input type="text" name="name[]" id="name_1" class="form-control" required="" readonly="" onkeyup="getTotal(1)" value="' . $name . '" autocomplete="off"></td>
                        <td><input type="text" name="qty[]" id="qty_1" class="form-control" required="" readonly="" onkeyup="getTotal(1)" value="' . $quantity . '" autocomplete="off"></td>
                        <td><input type="text" name="rate[]" id="rate_1" class="form-control" readonly="" value="' . get_instance()->formatprice($price) . '" autocomplete="off"></td>
                    </tr>';
            }
            $items_list .= '<div class="row"></div>';
        ...
        $order_information = '
        <div class="col-md-12">
            <h3>Cadastro de Devolução de Produtos</h3>
        </div>';

        $order_info = $this->model_product_return->getOrderInformation($order_id);
        ...
        $nfe_faturamento = $order_info['chave'];

        $order_information .= '
        <!-- Número do pedido Conecta Lá -->
        <div class="col-md-3 col-xs-12 form-group">
            <label>Pedido</label>
            <input type="text" name="order_conecta" class="form-control" value="' . $order_info['id'] . '" disabled>
        </div>

        <!-- Marketplace -->
        <div class="col-md-3 col-xs-12 form-group">
            <label>Marketplace</label>
            <input type="text" name="order_marketplace" class="form-control" value="' . $order_info['bill_no'] . '" disabled>
        </div>

        <!-- Loja -->
        <div class="col-md-3 col-xs-12 form-group">
            <label>Loja</label>
            <input type="text" name="store_name" class="form-control" value="' . $order_info['name'] . '" disabled>
        </div>

        <!-- Número da nota fiscal do pedido -->
        <div class="col-md-3 col-xs-12 form-group">
            <label>Número da nota fiscal</label>
            <input type="text" name="order_nfe" class="form-control" value="' . $order_info['nfe_num'] . '" disabled>
        </div>';

        $this->data['order_information'] = $order_information;
        $this->data['items_list'] = $items_list;
        $this->data['products_return'] = $products_return;
        $this->data['post_items'] = $post_items;
        $this->data['complete_order'] = $complete_order;
        $this->data['nfe_faturamento'] = $nfe_faturamento;
        $this->data['refund_on_gateway_value_to_return'] = $this->postClean('refund_on_gateway_value_to_return');

        // Lista de transportadoras.
        // $providers_list = $this->model_product_return->getProvidersList();
        $order_info = $this->model_orders->getOrdersData(0, $order_id);

        if (empty($order_info)) {
            redirect('ProductsReturn/return', 'refresh');
        }

        if (!empty($this->data['refund_on_gateway_value_to_return'])) {
            ...
            $max_to_refund = $order_info['total_order'] - ($total_refund + $total_return);

            if ($this->data['refund_on_gateway_value_to_return'] > $max_to_refund) {
        ...
        if ($this->ms_freight_tables->use_ms_shipping) {
            try {
                $this->ms_freight_tables->setStore($order_info['store_id']);
                $shipping_companies = $this->ms_freight_tables->getShippingCompanies();
            } catch (Exception $exception) {
                $shipping_companies = array();
            }
		...
		$this->data['ship_company_preview'] = '<option value="ESCOLHA" selected>Escolha</option><option value="TRANSPORTADORA">Transportadora</option>';

        $providers = "<option value='ESCOLHA' selected>Escolha</option>";
        if (strtolower($order_info['ship_company_preview']) == 'transportadora') {
            $transp_found = false;
            foreach ($shipping_companies as $shipping_company) {
                $this->data['ship_company_preview'] = '<option value="ESCOLHA">Escolha</option><option value="' . strtoupper($order_info['ship_company_preview']) . '" selected>' . $order_info['ship_company_preview'] . '</option>';

                $shipping_company_id    = $this->ms_freight_tables->use_ms_shipping ? $shipping_company->id : $shipping_company['id'];
                $shipping_company_name  = $this->ms_freight_tables->use_ms_shipping ? $shipping_company->name : $shipping_company['name'];

                if (!empty($item) && strtolower($item['logistic_operator_name']) == strtolower($shipping_company_name)) {
                    $transp_found = true;
                    $providers .= '<option value="' . $shipping_company_id . '" selected>' . $shipping_company_name . '</option>';
                } else {
                    $providers .= '<option value="' . $shipping_company_id . '">' . $shipping_company_name . '</option>';
                }
            }
        } else {       
            foreach ($shipping_companies as $shipping_company) {
        ...
        $this->data['ntransportadora'] = $providers;

        $this->render_template('orders/request_return', $this->data);
    }

    public function returnOrdersProvidersList(int $store_id)
    {
        $response = [];
        ...
        if ($this->ms_freight_tables->use_ms_shipping) {
            ...
            $shipping_companies = $this->model_product_return->getProvidersList();
        }
    ...
    public function nfeUpload()
    {
        $serverpath = $_SERVER['SCRIPT_FILENAME'];
        $pos = strpos($serverpath, 'assets');
        $serverpath = substr($serverpath, 0, $pos);
        $target_dir = $serverpath . 'assets/files/nfe/';

        if (!file_exists($target_dir)) {
            @mkdir($target_dir, 0766, true);
        }

        $new_name = "NFE-" . date("Ymdhis", strtotime("now")) . "-" . basename($_FILES["upload_nfe"]["name"]);

        $target_file = $target_dir . basename($new_name);
        $file_type = strtolower(pathinfo($new_name, PATHINFO_EXTENSION));

        $return = json_encode("fail");
        if (($file_type === "pdf") && ($_FILES["upload_nfe"]["size"] > 0)) {
            if (move_uploaded_file($_FILES["upload_nfe"]["tmp_name"], $target_file)) {
    ...
    public function letterUpload()
    {
        ...
        $new_name = "Letter-" . date("Ymdhis", strtotime("now")) . "-" . basename($_FILES["upload_letter"]["name"]);

        $target_file = $target_dir . basename($new_name);
        $file_type = strtolower(pathinfo($new_name, PATHINFO_EXTENSION));

        $return = json_encode("fail");
        if (($file_type === "pdf") && ($_FILES["upload_letter"]["size"] > 0)) {
            if (move_uploaded_file($_FILES["upload_letter"]["tmp_name"], $target_file)) {
                $return = json_encode($new_name);
            }
        }
    ...
    public function insertReturnedProduct()
    {
        $this->output->set_content_type('application/json');
        $postdata = $this->postClean();

        $this->log_data(__CLASS__, __FUNCTION__.'/'.$postdata['return_action'].'/body', json_encode($postdata, JSON_UNESCAPED_UNICODE));

        $order_id = $postdata['products_return'][0]["order_id"];
        $return_nfe_number = $postdata["return_nfe_number"];

        $nfes_order = $this->model_nfes->getNfesDataByOrderId($order_id);
        foreach ($nfes_order as $nfe_order) {
            if ($nfe_order['nfe_num'] == $return_nfe_number) {
                return $this->output->set_output(json_encode(array('success' => false, 'message' => $this->lang->line('messages_order_invoice_already_exists')), JSON_UNESCAPED_UNICODE));
            }
        }
        ...
        $refund_on_gateway_value_to_return = $this->postClean('refund_on_gateway_value_to_return');
        if (!empty($refund_on_gateway_value_to_return)) {
            /*if (
                $days_to_refund_value_tuna &&
                strtotime(addDaysToDate($order['date_time'], $days_to_refund_value_tuna)) < strtotime(dateNow()->format(DATETIME_INTERNATIONAL))
            ) {
            ...
            $max_to_refund = $order['total_order'] - ($total_refund + $total_return);

            if ($refund_on_gateway_value_to_return > $max_to_refund) {
                $this->session->set_flashdata('error', $this->lang->line('messages_mx_value_not_allowed'));
                redirect("orders/update/$order_id", 'refresh');
            }
            ...
            $this->model_order_value_refund_on_gateways->create(array(
                ...
                'user_id'       => $this->session->userdata('id'),
                'value'         => $refund_on_gateway_value_to_return,
                'description'   => "Criado via devolução de produtos",
                'is_product_return' => true
            ));
        } else {
            if ($days_to_refund_value_tuna) {
                ...
                for ($index = 0; $index < count($product['products_return']); $index++) {
                    ...
                    $value_refund += $return_total_value;

                    $order_values_to_refund[] = array(
                        'product_value' => $return_total_value,
                        'shipping_value' => 0
                    );
                }
        ...
        // É uma criação de devolução e aconteceu a criação.
        if ($inserted && (empty($postdata['return_action']) || $postdata['return_action'] == "create")) {
            $this->model_orders->updateByOrigin($order_id, array('paid_status' => $this->model_orders->PAID_STATUS['in_refund']));
        }

        return $this->output->set_output(json_encode(array('success' => true, 'message' => $postdata['return_action'] == "create" ? $this->lang->line('messages_successfully_created') : $this->lang->line('messages_successfully_updated')), JSON_UNESCAPED_UNICODE));
    }
    ...
    public function updateStatus()
    {
        ...
        if (
            (isset($postdata['return_id']) && ($postdata['return_id'] != "")) && 
            (isset($postdata['status']) && ($postdata['status'] != ""))
        ) {
            $this->log_data(__CLASS__, __FUNCTION__, json_encode($postdata, JSON_UNESCAPED_UNICODE));

            $data_return = $this->model_product_return->getById($postdata['return_id']);

            if (in_array($data_return['status'], array('cancelado', 'devolvido'))) {
                return $this->output
                    ->set_content_type('application/json')
                    ->set_output(json_encode(array(
                        'success'   => false,
                        'message'   => "Devolução com status de $data_return[status], não é mais permitido atualizações."
                    )));
            }

            $this->model_product_return->updateStatus($postdata['return_id'], $postdata['status']);

            if (array_key_exists('order_id', $postdata)) {
                $all_completed = true;
                // Verifica se todos os produtos já estão concluídos(devolvido/cancelado).
                foreach ($this->model_product_return->getByOrderId($postdata['order_id']) as $product_refunded) {
                    if (in_array($product_refunded['status'], array(Model_product_return::COLLECTED, Model_product_return::TO_HIRE))) {
                        $all_completed = false;
                    }
                }

                if ($all_completed) {
                    $this->model_orders->updateByOrigin($postdata['order_id'], array('paid_status' => $this->model_orders->PAID_STATUS['refunded_sent_to_marketplace']));
                }
            }
        }
    ...
    public function returnMassive()
    {

        if (!in_array('updateOrdersCancelCommissionCharges', $this->permission)) {
            redirect('dashboard', 'refresh');
        }

        $this->data['page_title'] = $this->lang->line('application_massive_order_refund');
        $this->data['users'] = $this->model_order_refund_massive->getUserList();

        $this->render_template('products_return/return_massive', $this->data);

    }

    public function returnMassiveDetails($id)
    {
        ...
        $this->data['page_title'] = $this->lang->line('application_massive_order_refund');
        $this->data['users'] = $this->model_order_refund_massive->getUserList();
        $this->data['entry'] = $this->model_order_refund_massive->findByPk($id);
        if (!$this->data['entry']){
            redirect('ProductsReturn/returnMassive', 'refresh');
        }
        $this->data['entry']['errors'] = json_decode($this->data['entry']['errors'],true);

        $this->render_template('products_return/return_massive_details', $this->data);

    }

    public function returnMassiveCreate()
    {
        if (!in_array('updateOrdersCancelCommissionCharges', $this->permission)) {
            redirect('dashboard', 'refresh');
        }
        ...
        $this->render_template('products_return/import_file', $this->data);

    }

    public function uploadReturnMassiveCreate()
    {
        ...
        $path = 'assets/files/orders_refund_massive/';
        if (!is_dir($path)) {
            @mkdir($path, 0775);
        }
        ...
        if (!$this->upload->do_upload('csvFile')) {
            ...
            $csv_filename = $this->upload->data('file_name');

            $config['allowed_types'] = '*';
            $this->upload->initialize($config);
            if (!$this->upload->do_upload('justifyFile')) {
                exit(json_encode(['status' => false, 'errors' => $this->upload->display_errors()]));
            } else {
                $justify_filename = $this->upload->data('file_name');
            }

            $item = [
                'orders_file' => "{$path}{$csv_filename}",
                'justify_file' => "{$path}{$justify_filename}",
                'status' => OrderRefundMassiveStatus::READY,
                'user' => $this->session->userdata('email'),
            ];

            if ($this->model_order_refund_massive->create($item)) {
    ...
    public function returnMassiveItens()
    {
        ...
        $data = $this->model_order_refund_massive->getList($postdata);
        $recordsTotal = $this->model_order_refund_massive->countTotalList($postdata);
    ...
    public function returnMassiveDownloadErrors($id)
    {
        ...
        $entry = $this->model_order_refund_massive->findByPk($id);
        $errors = json_decode($entry['errors'], true);
        $errorsItens = $errors['itens'];
        if (!$errorsItens){
            redirect('ProductsReturn/returnMassive', 'refresh');
        }
        ...
        $column = 0;
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $line, 'ID do Pedido');
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(++$column, $line, lang('application_error'));

        foreach ($errorsItens as $orderId => $message){
            $line++;
            $column = 0;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $line, (string)$orderId);
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(++$column, $line, $message);
        }

        $filename = "products_return_massive_".date("Y-m-d-H-i-s").".xlsx";
    ...
    private function generateReturnDataFromArrayResult(array $data): array
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $fileUrl = base_url($value['orders_file']);
            $status = OrderRefundMassiveStatus::getName($value['status']);
            $statusColor = OrderRefundMassiveStatus::getColor($value['status']);
            $action = "";
            if ($value['status'] == OrderRefundMassiveStatus::ERROR){
                $actionUrl = base_url('ProductsReturn/returnMassiveDetails/'.$value['id']);
                $action = "<a href='$actionUrl' class='btn bg-blue'><i class='far fa-eye'></i></a>";
            }

            $result[$key] = [
                'id' => $value['id'],
                'file' => "<a href='{$fileUrl}'>Baixar Arquivo <i class='fa fa-download'></i></a>",
                'status' => "<span class='label {$statusColor}'>{$status}</span>",
                'user' => $value['user'],
                'created_at' => datetimeBrazil($value['created_at']),
                'actions' => $action,
            ];
