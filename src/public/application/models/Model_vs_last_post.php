...
class Model_vs_last_post extends CI_Model
{
	...
	{
		if($id) {
			$sql = "SELECT * FROM vs_last_post WHERE id = ?";
			$query = $this->db->query($sql, array($id));
			return $query->row_array();
		}

		$sql = "SELECT * FROM vs_last_post";
	...
	{
		if($data) {
			$insert = $this->db->insert('vs_last_post', $data);
	...
	public function update($data, $id)
	{
		if($data && $id) {
			$this->db->where('id', $id);
			$update = $this->db->update('vs_last_post', $data);
	...
	public function remove($id)
	{
		if($id) {
			$this->db->where('id', $id);
			$delete = $this->db->delete('vs_last_post');
	...
	{
		if($data) {
			$insert = $this->db->replace('vs_last_post', $data);
	...
	{
		if (is_null($variant)) {
			$sql = "SELECT * FROM vs_last_post WHERE int_to= ? AND prd_id = ? ";
			$query = $this->db->query($sql, array($int_to, $prd_id));
		}
		else {
			$sql = "SELECT * FROM vs_last_post WHERE int_to= ? AND prd_id = ? AND variant = ?";
	...
	{
		$sql = "SELECT * FROM vs_last_post WHERE skulocal = ? ";
	...
	{
		$sql = "UPDATE vs_last_post SET qty=qty-?, qty_total=qty_total-? WHERE id = ? ";
	...
	public function verifyProductOnSeller($seller_id, $sku)
	{
		$sql = "SELECT * FROM vs_last_post WHERE skulocal = ? AND seller_id = ? LIMIT 1";
