...
class Model_ml_ult_envio extends CI_Model
{
	...
	{
		if($id) {
			$sql = "SELECT * FROM ml_ult_envio WHERE id = ?";
			$query = $this->db->query($sql, array($id));
			return $query->row_array();
		}

		$sql = "SELECT * FROM ml_ult_envio";
	...
	{
		if($data) {
			$insert = $this->db->insert('ml_ult_envio', $data);
	...
	public function update($data, $id)
	{
		if($data && $id) {
			$this->db->where('id', $id);
			$update = $this->db->update('ml_ult_envio', $data);
	...
	public function remove($id)
	{
		if($id) {
			$this->db->where('id', $id);
			$delete = $this->db->delete('ml_ult_envio');

            try {
                if ($this->ms_shipping->use_ms_shipping) {
                    $ml_ult_envio = $this->getData($id);
                    if ($ml_ult_envio) {
                        $this->ms_shipping->removePivot($ml_ult_envio);
                    }
                }
            } catch (Exception $exception) {}
	...
	{
		if($data) {
			$insert = $this->db->replace('ml_ult_envio', $data);
	...
	{
		if (is_null($variant)) {
			$sql = "SELECT * FROM ml_ult_envio WHERE int_to= ? AND prd_id = ? ";
			$query = $this->db->query($sql, array($int_to, $prd_id));
		}
		else {
			$sql = "SELECT * FROM ml_ult_envio WHERE int_to= ? AND prd_id = ? AND variant = ?";
	...
	{
		$sql = "SELECT * FROM ml_ult_envio WHERE skulocal = ? ";
	...
	{
		$sql = "UPDATE ml_ult_envio SET qty=qty-?, qty_total=qty_total-? WHERE id = ? ";
