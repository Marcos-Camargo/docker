...
class Model_fields_orders_mandatory extends CI_Model
{
    ...
    public function getFieldsOrdersMandatory($store_id){
        // consulta os campos obrigatórios da loja (marcados com checkbox)
        return $this->db->select('tid, nsu, authorization_id, first_digits, last_digits')
                    ->where('store_id', $store_id)
                    ->get('fields_orders_mandatory')
                    ->row_array() ?? []; // se não tiver resultado, retorna array vazio
    }
