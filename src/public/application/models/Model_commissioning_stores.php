...
class Model_commissioning_stores extends CI_Model
{

    private $tableName = 'commissioning_stores';
    private $createLog;
    ...
    public function getCommissionByStoreAndDateRange(string $int_to, int $store_id, string $order_date): ?array
    {
        return $this->db->select('c.id, cs.comission')
            ->join('commissionings c', 'c.id = cs.commissioning_id')
            ->where(array(
                'cs.store_id' => $store_id,
                'c.int_to =' => $int_to,
                'c.start_date <=' => $order_date,
                'c.end_date >=' => $order_date
            ))->get($this->tableName.' cs')
            ->row_array();
    }

    public function getCommissionDateRange(string $int_to, int $store_id, string $dateStart, string $dateEnd): ?array
    {
        return $this->db->select('cs.comission')
            ->join('commissionings c', 'c.id = cs.commissioning_id')
            ->where(array(
                'cs.store_id' => $store_id,
                'c.int_to =' => $int_to,
    ...
    private function generateQueryFromToFindItens(
        ...
        $sql = ",commissioning_stores.store_id, commissioning_stores.comission FROM commissionings 
        JOIN commissioning_stores ON (commissioning_stores.commissioning_id = commissionings.id)";
        $sql .= " WHERE commissionings.type = '".ComissioningType::SELLER."' ";

        if ($store_id) {
            $sql .= " AND commissioning_stores.store_id = $store_id ";
        }
        if ($int_to) {
