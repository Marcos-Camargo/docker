...
class Model_legal_panel extends CI_Model
{
    private $tableName = 'legal_panel';
    ...
    {
        $sql = "SELECT lp.*, 
                       COALESCE(lp.lote, c.lote) AS lote
                FROM legal_panel lp
                LEFT JOIN conciliacao c 
                    ON lp.conciliacao_id = c.id 
                    AND lp.lote IS NULL
                WHERE lp.id = ?
";
    ...
    {
        $sql = "SELECT * FROM `legal_panel` WHERE store_id = ? AND balance_debit = ? AND conciliacao_id = ?";
	...
	public function getAllBetweenDate(string $dateStart = null, string $dateEnd = null)
	{
        ...
        $this->db->join('stores AS store_legal_panel', 'store_legal_panel.id = legal_panel.store_id', 'left');
    ...
    {
        if($data) {
            $insert = $this->db->insert('legal_panel', $data);
	...
	public function update($data, $id)
    {
        if($data && $id) {
            $this->db->where('id', $id);
            $update = $this->db->update('legal_panel', $data);
            if (isset($this->session)){
	...
	public function remove($id)
    {
        if($id) {
            $this->db->where('id', $id);
            $delete = $this->db->delete('legal_panel');
    ...
    public function getAPiListItems($filters_array): array
    {
        ...
        $sql_total_rows = "select count(id) as total_rows from legal_panel where 1=1 ".$sql;
    ...
    public function getRefundByStoreIdAndLote(int $store_id, string $lote): ?array
    {
        return $this->db->select('sum(cs.valor_repasse) as sum_valor_repasse')
            ->join('legal_panel lp', 'cs.legal_panel_id = lp.id')
            ->where(array(
                'cs.store_id' => $store_id,
                'cs.lote' => $lote,
                'cs.status_conciliacao' => 'Conciliação Painel Jurídico',
                'lp.notification_id' => 'Devolução de produto.',
            ))
            ->get('conciliacao_sellercenter cs')
            ->row_array();
    }
