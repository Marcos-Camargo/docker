...
class Model_commissioning_categories extends CI_Model
{

    private $tableName = 'commissioning_categories';
    private $createLog;
    ...
    private function generateQueryFromToFindItens(
        ...
        $sql = ",stores.name as store_name,commissioning_categories.store_id, commissioning_categories.comission FROM commissionings 
        JOIN commissioning_categories ON (commissioning_categories.commissioning_id = commissionings.id)
        JOIN stores ON (stores.id = commissioning_categories.store_id)";
        $sql .= " WHERE commissionings.type = '".ComissioningType::CATEGORY."' ";

        if ($store_id) {
            $sql .= " AND commissioning_categories.store_id = $store_id ";
        }
        if ($int_to) {
    ...
    public function getCommissionByCategoryAndStoreAndDateRange(
        string $int_to,
        int $category_id,
        int $store_id,
        string $order_date
    ): ?array {
        return $this->db->select('c.id, cc.comission')
            ->join('commissionings c', 'c.id = cc.commissioning_id')
            ->where(array(
            ...
            ))->get('commissioning_categories cc')
            ->row_array();
    }

    public function getCommissionByCategoryDateRange(
        int $store_id,
        string $int_to,
        int $category_id,
        string $dateStart,
        ...
        return $this->db->select('cc.comission')
            ->join('commissionings c', 'c.id = cc.commissioning_id')
            ->where(array(
                'cc.category_id' => $category_id,
                'cc.store_id' => $store_id,
