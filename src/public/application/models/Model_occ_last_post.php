...
class Model_occ_last_post extends CI_Model
{
	...
	{
		if($id) {
			$sql = "SELECT * FROM occ_last_post WHERE id = ?";
			$query = $this->db->query($sql, array($id));
			return $query->row_array();
		}

        if (!is_null($procura)) {
            $query = $this->db->query("SELECT * FROM occ_last_post $procura");
            return $query->row_array();
        }

		$sql = "SELECT * FROM occ_last_post";
	...
	{
		if($data) {
			$insert = $this->db->insert('occ_last_post', $data);
	...
	public function update($data, $id)
	{
		if($data && $id) {
			$this->db->where('id', $id);
			$update = $this->db->update('occ_last_post', $data);
	...
	public function remove($id)
	{
		if($id) {
			$this->db->where('id', $id);
			$delete = $this->db->delete('occ_last_post');
	...
	{
		if($data) {
			$insert = $this->db->replace('occ_last_post', $data);
	...
	{
		if (is_null($variant)) {
			$sql = "SELECT * FROM occ_last_post WHERE int_to= ? AND prd_id = ? ";
			$query = $this->db->query($sql, array($int_to, $prd_id));
		}
		else {
			$sql = "SELECT * FROM occ_last_post WHERE int_to= ? AND prd_id = ? AND variant = ?";
	...
	{
		$sql = "SELECT * FROM occ_last_post WHERE skulocal = ? ";
	...
	{
		$sql = "UPDATE occ_last_post SET qty=qty-?, qty_total=qty_total-? WHERE id = ? ";
	...
	{
		if (is_null($variant)) {
			$sql = "SELECT * FROM occ_last_post WHERE int_to=? AND prd_id = ? AND variant is null";
			$query = $this->db->query($sql, array($int_to,$prd_id));
		}
		else {
			$sql = "SELECT * FROM occ_last_post WHERE int_to=? AND prd_id = ? AND variant = ?";
