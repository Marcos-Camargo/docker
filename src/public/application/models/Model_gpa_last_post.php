...
class Model_gpa_last_post extends CI_Model
{
	...
	{
		if($id) {
			$sql = "SELECT * FROM gpa_last_post WHERE id = ?";
			$query = $this->db->query($sql, array($id));
			return $query->row_array();
		}

		$sql = "SELECT * FROM gpa_last_post";
	...
	{
		if($data) {
			$insert = $this->db->insert('gpa_last_post', $data);
	...
	public function update($data, $id)
	{
		if($data && $id) {
			$this->db->where('id', $id);
			$update = $this->db->update('gpa_last_post', $data);
	...
	public function remove($id)
	{
		if($id) {
			$this->db->where('id', $id);
			$delete = $this->db->delete('gpa_last_post');
	...
	{
		if($data) {
			$insert = $this->db->replace('gpa_last_post', $data);
	...
	{
		if (is_null($variant)) {
			$sql = "SELECT * FROM gpa_last_post WHERE int_to= ? AND prd_id = ? ";
			$query = $this->db->query($sql, array($int_to, $prd_id));
		}
		else {
			$sql = "SELECT * FROM gpa_last_post WHERE int_to= ? AND prd_id = ? AND variant = ?";
	...
	{
		$sql = "SELECT * FROM gpa_last_post WHERE skulocal = ? ";
	...
	{
		$sql = "UPDATE gpa_last_post SET qty=qty-?, qty_total=qty_total-? WHERE id = ? ";
    ...
    {
    	$sql = "SELECT * FROM gpa_last_post WHERE skulocal = ? AND int_to= ?";
