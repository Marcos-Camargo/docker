...
Model de Acesso ao BD para carrefour_new_offers

*/  

class Model_carrefour_new_offers extends CI_Model
{
	...
	{
		if($skulocal) {
			$sql = "SELECT * FROM carrefour_new_offers WHERE skulocal= ?";
			$query = $this->db->query($sql, array($skulocal));
			return $query->row_array();
		}

		$sql = "SELECT * FROM carrefour_new_offers";
	...
	{
		if($data) {
			$insert = $this->db->insert('carrefour_new_offers', $data);
	...
	public function update($data, $skulocal)
	{
		if($data && $skulocal) {
			...
			$update = $this->db->update('carrefour_new_offers', $data);
	...
	public function remove($skulocal)
	{
		if($skulocal) {
			$this->db->where('skulocal', $skulocal);
			$delete = $this->db->delete('carrefour_new_offers');
	...
	{
		if($data) {
			$insert = $this->db->replace('carrefour_new_offers', $data);
	...
	public function createIfNotExist($skulocal, $data)
	{
		...
		$sql = "SELECT skulocal FROM carrefour_new_offers WHERE skulocal= ?";
		$query = $this->db->query($sql, array($skulocal));
		$row = $query->row_array();
		if ($row) {
	...
	{
		$sql = "SELECT * FROM carrefour_new_offers WHERE status=0 AND store_id=?";
