...
class Intelipost extends Logistic
{
    ...
    public function getPriceAndDeadlineIntelipost(array $skus, ?int $cross_docking, string $zip, ?string $quoteId, ?string $shipService): array
    {
            ...
            try {
                ...
                if (isset($contentOrder->content->delivery_options) && is_array($contentOrder->content->delivery_options) && count($contentOrder->content->delivery_options)) {
                    foreach ($contentOrder->content->delivery_options as $service) {
                        if (strtolower($service->delivery_method_name) == strtolower($shipService)) {
                            return array(
                ...
                $message = $message_decode['messages'][0]['text'] ?? $message;
                $message_key = $message_decode['messages'][0]['key'] ?? null;
                // cotação já expirou ou não existe mais.
                if ($message_key !== 'quote.id.unknown') {
        ...
        try {
            ...
            $message = $exception->getMessage();
            $message = json_decode($message, true)['messages'][0]['text'] ?? $message;
            throw new InvalidArgumentException($message);
        }
     ...
     * @return  array
     */
    public function getQuoteAsync(array $dataQuote, bool $moduloFrete = false): array
    {
        $sales_channel = $this->sellerCenter;
        if (!$this->freightSeller) {
            $sales_channel .= "-$this->store";
        ...
        foreach ($dataQuote['items'] as $sku) {
            ...
            if ($moduloFrete) {
                ...
                $arrQuote['products'] = array($dataProduct);

                $promises["{$arrSkuProductId['skumkt']}:{$arrSkuProductId['prd_id']}"] = array('json' => $arrQuote);
            }
            // Não é módulo frete, então montarei um vetor para fazer a cotação de todos ao mesmo tempo.
            else {
                $arrQuote['products'][] = $dataProduct;
            }
        }
        ...
        if (!$moduloFrete) {
            ...
            try {
                $responses = $this->requestAsync('POST', "/quote_by_product", $promises);

                foreach ($responses as $skumkt_product => $content_response) {
                    $services = array_merge($services, $this->getQuoteUnitAsync($content_response, explode(':', $skumkt_product)));
                }
    ...
    public function getLabelIntelipost(array $freight, string $tracking, array $order, int $volume, int $inResendActive): void
    {
        ...
        if($inResendActive == 0){
            $etiquetaA4  =      "$pathEtiquetas/P_{$order['id']}_{$volume}_A4.pdf";
            $etiquetaThermal =  "$pathEtiquetas/P_{$order['id']}_{$volume}_Termica.pdf";
        }else{
        ...
        try {
            ...
            $contentOrder = Utils::jsonDecode($response->getBody()->getContents());
            get_instance()->log_data('batch', $log_name, "\nPEDIDO={$order['id']} gerando etiquetas.\ncontent_label= " . Utils::jsonEncode($contentOrder));
        } catch (InvalidArgumentException $exception) {
            throw new InvalidArgumentException($exception->getMessage());
        }
    ...
    public function hireFreight(array $order, array $store, array $nfe, array $client, bool $orderWaitTrackingCode)
    {
        ...
        if ($orderWaitTrackingCode) {
            $this->getAllLabelIntelipost($order, $nfe);
            return;
        }
        ...
        foreach ($items as $iten) {
            $items_cancel = $this->model_order_items_cancel->getByOrderIdAndItem($order['id'], $iten['id']);
            if ($items_cancel) {
                if ($items_cancel['qty'] != $iten['qty']) {
                    $iten['qty'] = $iten['qty'] - $items_cancel['qty'];
                } else {
        ...
        // faço a cotação para pegar os dados de atualizações.
        $quote_ship_data = $this->model_quotes_ship->getQuoteShipByOrderId($order['id']);
        try {
            $quote_id = $quote_ship_data['token_oferta'] ?? null;
            if ($quote_id === '{}' && !empty($quote_ship_data['retorno']) && $quote_ship_data['retorno'] !== '{}') {
                $return_decode = json_decode($quote_ship_data['retorno'], true);
                $quote_id = $return_decode['data']['services'][0]['quote_id'] ?? $quote_id;
            }
        ...
        $intelipost = array(
            ...
            "end_customer" => [ // Informações do cliente final
                ...
                "shipping_city" 		=> $order['customer_address_city'],
        ...
        if (!$this->freightSeller) {
            ...
            $intelipost['seller'] = array(
                "first_name"            => $store['name'],
                "last_name"             => $store['name'],
                "email"                 => $store['responsible_email'],
                "phone"                 => $store['phone_1'],
                "cellphone"             => $store['phone_2'],
                "is_company"            => strlen(onlyNumbers($store['CNPJ'])) === 14,
                "federal_tax_payer_id"  => onlyNumbers($stores['CNPJ']),
                "state_tax_payer_id"    => onlyNumbers($stores['inscricao_estadual']),
                "country"               => $store['country'],
                "state"                 => $store['addr_uf'],
                "city"                  => $store['addr_city'],
                "address"               => $store['address'],
                "number"                => $store['addr_num'],
                "quarter"               => $stores['addr_neigh'],
                "reference"             => "",
                "additional"            => "",
        ...
        foreach ($arrTracking as $index => $tracking) {
            ...
            if ($isCorreios) {
                get_instance()->log_data('batch',$log_name, "\nPedido ( {$order['id']} ) é frete correios verificando variavel .\nVariavel preShipmentListState " . $preShipmentListState);
                if ($preShipmentListState  == 'SUCCESS') {
                    try {
                        ...
                        get_instance()->log_data('batch', $log_name, "\nPEDIDO={$order['id']} gerando etiquetadas. \ncontent_label= " . Utils::jsonEncode($contentLabel));
                    } catch (InvalidArgumentException $exception) {
                        throw new InvalidArgumentException($exception->getMessage());
                    }
                    ...
                    if (is_file(FCPATH . "$pathEtiquetas/P_{$order['id']}_{$tracking['volume_number']}_A4.pdf") && is_file(FCPATH . "$pathEtiquetas/P_{$order['id']}_{$tracking['volume_number']}_Termica.pdf")) {
                        ...
                        echo "Pedido {$order['id']} intelipost, etiqueta gerada e salva.\n";
                        get_instance()->log_data('batch',$log_name, "Pedido ( {$order['id']} ) etiqueta baixada intelipost.\ncontent=" . Utils::jsonEncode($contentLabel));

                    }else{
                        echo "Pedido {$order['id']} intelipost, etiqueta não gerada.\n";
                        get_instance()->log_data('batch',$log_name, "Pedido ( {$order['id']} ) não gravou etiqueta intelipost no servidor. \ncontent=" . Utils::jsonEncode($contentLabel));
                    }
                }
            }
        ...
        $this->model_orders->updatePaidStatus($order['id'], !empty($tracking['tracking']) ? 51 : 50);
    ...
    public function getAllLabelIntelipost(array $order, array $nfe): void
    {
        $freightOrders = $this->model_freights->getDataFreightsOrderId($order['id'], true, false);

        $trackingAlreadyRead = array();

        $log_name = __CLASS__.'/'.__FUNCTION__;
        if ($freightOrders) {
            $freightOrder = $freightOrders[0];
            ...
            foreach ($contentOrder->content as $content) {
                ...
                if (!isset($content->shipment_order_volume_array)) {
                    echo "Pedido {$order['id']} intelipost, sem informações sobre volume\n";
                    continue;
                }

                $allSuccess = true;
                foreach ($content->shipment_order_volume_array as $tracking) {
                    if ($tracking->pre_shipment_list_state != 'SUCCESS') {
                        $allSuccess = false;
                        break;
                    }
                }

                if (!$allSuccess || !in_array($order['paid_status'], [50, 80])) {
                    echo "Pedido {$order['id']} intelipost, ainda sem disparo de PLP\n";
                    continue;
                }

                foreach ($content->shipment_order_volume_array as $tracking) {
                    $trackingCode = $tracking->tracking_code ?: $nfe['nfe_num'];
