...
class RedisCodeigniter {
    ...
    public function configure($params = array())
    {
        ...
        // Connect to Redis
        $this->_connection = @fsockopen($config['host'], $config['port'], $errno, $errstr, $params['timeout'] ?? 3);

        // Display an error message if connection failed
        if ( ! $this->_connection)
        {
            log_message('error', 'Could not connect to Redis at ' . $config['host'] . ':' . $config['port'] . ': ' . $errstr . ' (' . $errno . ')');
    ...
    {
        // SENTRY ID: 590, 587, 579, 576, 574, 571, 567, 564, 561, 558, 555, 552, 549, 546, 543, 538, 534, 531, 528, 526, 522, 519, 516, 513
    ...
    protected function reconnect()
    {
        ...
        // Fecha a conexão antiga se ainda existir
        if (is_resource($this->_connection)) {
        ...
        //Reconectando
        $this->configure(self::$connectionParams);
    ...
    private function _write_request($request)
    {
        ...
        // SENTRY ID: 588, 585, 578, 573, 570, 566, 562, 559, 556, 553, 550, 547, 544, 542, 536, 532, 529, 527, 521, 517, 514, 511, 508
        if (is_resource($this->_connection)) {
            if ($value_length <= 8192)
            {
                ...
                if ($result === false) {
                    // Tenta reconectar e tentar de novo
                    $this->reconnect();

                    // Tenta novamente escrever
                    $result = @fwrite($this->_connection, $request);

                    if ($result === false) {
                        // Falhou mesmo após reconectar
                        throw new Exception("Falha ao escrever no Redis: broken pipe mesmo após reconexão.");
                    }
                }
            }
            else
            {
                while ($value_length > 0)
                {
                    ...
                    // Send our chunk
                    $result = @fwrite($this->_connection, $request, $send_size);
            ...
            log_message('error', 'Redis connection is not a resource in _write_request.');
            return FALSE;
        }
    ...
    {
        // SENTRY ID: 589, 586, 577, 575, 572, 569, 565, 563, 560, 557, 554, 551, 548, 545, 541, 539, 535, 533, 530, 525, 524, 520, 518, 515, 512, 509
        if (!is_resource($this->_connection)) {
            log_message('error', 'Redis connection is not a resource in _read_request.');
            return FALSE;
        }
