...
class Sqltoci {
    ...
    function generate($tables = null) {
        ...
        if ($this->write_file)
        {
            ...
            $file_path = $path . '/'.date('YmdHis')."_add_{$this->tables[0]}.php";
            $file = fopen($file_path, 'w+');
            ...
            foreach ($columns as $column)
            {
                ...
                if ( strpos($column['Type'], '(') )
                {
                    ...
                    else
                    {
                        ...
                        $column_constraint = str_replace(')', '', $column['Type']);
                    }
                }
                else
                {
        ...
        $return .= "return new class extends CI_Migration
{" . "\n";
        $return .= "\n\t" . 'public function up() {';
        ...
        if ($this->write_file)
        {
            fwrite($file, $return);
            fclose($file);
            echo "Create file $file_path migration with success!".PHP_EOL;
            return true;
        } else
        {
            return $return;
    ...
    function generateClean($name = null) {
        ...
        if ($this->write_file)
        {
            ...
            $file_path = $path . '/'.date('YmdHis')."_{$name}.php";
            $file = fopen($file_path, 'w+');
